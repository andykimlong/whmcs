...
if ($action == "delete") {
    ...
    delete_query("tblcancelrequests", ["id" => $id]);
    redir();
} else {
    if ($action == "detail") {
        ...
        $data = [[AdminLang::trans("fields.ordernum") => $order ? $order->orderNumber : "", AdminLang::trans("fields.regdate") => $service ? $service->registrationDate->toAdminDateFormat() : ""], [AdminLang::trans("fields.recurringamount") => formatCurrency($service->amount), AdminLang::trans("fields.billingcycle") => $service->billingCycle], [AdminLang::trans("fields.nextduedate") => $nextDueOutput, AdminLang::trans("fields.status") => $service->domainStatus]];
        $aInt->jsonResponse(["output" => WHMCS\View\Bootstrap::renderKeyValuePairsInColumns(4, $data)]);
    }
}
...
if (!$filter) {
    if ($completed) {
        $query->whereIn("tblhosting.domainstatus", ["Cancelled", "Terminated"]);
    } else {
        $query->whereNotIn("tblhosting.domainstatus", ["Cancelled", "Terminated"]);
    }
}
$numrows = $query->count();
$query->select(["tblcancelrequests.*", "tblhosting.domain", "tblhosting.nextduedate", "tblproducts.name AS productname", "tblproductgroups.name AS groupname", "tblhosting.id AS productid", "tblhosting.userid", "tblclients.firstname", "tblclients.lastname", "tblclients.companyname", "tblclients.groupid"]);
$query->selectRaw("if (\n    tblhosting.nextduedate = '0000-00-00' or tblcancelrequests.type = 'Immediate',\n    substr(tblcancelrequests.date,1,10),\n    tblhosting.nextduedate\n) as sortdate");
$query->orderByRaw("sortdate asc, date asc");
$query->limit($limit);
$offset = 0;
if (1 <= $page) {
...
foreach ($result as $data) {
    ...
    $tabledata[] = [$date, $xname, "<textarea rows=\"3\" class=\"form-control\" readonly>" . $reason . "</textarea>", $type, $cancellationDate, $expandIcon, "<a href=\"#\" onClick=\"doDelete('" . $id2 . "');return false\"><img src=\"images/delete.gif\" width=\"16\" height=\"16\" border=\"0\" alt=\"" . $aInt->lang("global", "delete") . "\"></a>"];
}
echo $aInt->sortableTable([AdminLang::trans("fields.date"), AdminLang::trans("fields.product"), AdminLang::trans("fields.reason"), AdminLang::trans("fields.type"), AdminLang::trans("global.cancellationDate"), "", ""], $tabledata);

    ...
    if ($id) {
        if ($hours != 0) {
            if (preg_match("/ " . $hoursText . " @/", $description) || preg_match("/ " . $qtyText . " [0-9]+(\\.[0-9]{1,2})? @/", $description)) {
                ...
                if ($unit == 0) {
                    $description = sprintf("%s - %0.1f %s @ %0.2f/%s", $title, $hours, $hoursText, $amount, $hourText);
                } else {
                    $description = sprintf("%s - %s %0.1f @ %0.2f/%s", $title, $qtyText, $hours, $amount, $eachText);
                }
            }
            $amount = $amount * $hours;
        }
        ...
        if ($hours != 0) {
            if ($unit == 0) {
                $description .= sprintf(" - %0.1f %s @ %0.2f/%s", $hours, $hoursText, $amount, $hourText);
            } else {
                $description .= sprintf(" - %s %0.1f @ %0.2f/%s", $qtyText, $hours, $amount, $eachText);
            }
            $amount = $amount * $hours;
        }
...
if (!$action) {
    ...
    echo $aInt->sortableTable(["checkall", AdminLang::trans("fields.id"), AdminLang::trans("fields.description"), AdminLang::trans("fields.hours"), AdminLang::trans("fields.amount"), AdminLang::trans("billableitems.invoiceaction"), "", ""], $tabledata, $tableformurl, $tableformbuttons);

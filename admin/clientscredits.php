...
$result = select_query("tblclients", "firstname,lastname,credit", ["id" => $userid]);
$data = mysql_fetch_array($result);
$name = stripslashes($data["firstname"] . " " . $data["lastname"]);
$creditbalance = $data["credit"];
$errorMessages = [];
if ($action == "") {
    $validateAmountOn = ["add", "remove"];
    if (in_array($sub, $validateAmountOn)) {
        $validate = new WHMCS\Validate();
        if (!$validate->validate("decimal", "amount", ["credit", "invalidAmountFormat"])) {
            $errorMessages = array_merge($validate->getErrors());
        }
    }
    if (in_array($sub, array_merge($validateAmountOn, ["save"])) && !validateDateInput($date)) {
        $errorMessages[] = $aInt->lang("credit", "invalidDate");
    }
    if (empty($errorMessages)) {
        if (in_array($sub, ["add", "remove", "save", "delete"])) {
        ...
        if (in_array($sub, $validateAmountOn)) {
            ...
            if ($sub === "remove" && $creditbalance < $amount) {
                ...
                localAPI("AddCredit", ["clientid" => $userid, "adminid" => $adminId, "date" => toMySQLDate($date), "description" => $description, "amount" => $amount, "type" => $sub], $adminId);
                redir("userid=" . $userid);
            }
        }
        if ($sub == "save") {
            $update = ["date" => toMySQLDate($date), "description" => $description];
        ...
        if ($sub == "delete") {
            ...
            $result = select_query("tblcredit", "", ["id" => $ide]);
            $data = mysql_fetch_array($result);
            if ($data["clientid"] == $userid) {
                $amount = $data["amount"];
                if ($amount <= $creditbalance) {
                    $creditbalance = $creditbalance - $amount;
                    update_query("tblclients", ["credit" => $creditbalance], ["id" => (int) $userid]);
                    delete_query("tblcredit", ["id" => $ide, "clientid" => $userid]);
...
if ($action == "" || $action == "delete") {
    ...
    $result = select_query("tblcredit", "", ["clientid" => $userid], "date", "DESC");
    while ($data = mysql_fetch_array($result)) {
        ...
        $tabledata[] = [$date, nl2br(trim($description)), formatCurrency($amount), $adminName, $editLink, $deleteLink];
    }
    echo $aInt->sortableTable([AdminLang::trans("fields.date"), AdminLang::trans("fields.description"), AdminLang::trans("fields.amount"), AdminLang::trans("fields.admin"), "", ""], $tabledata);
    ...
    if ($action == "add" || $action == "remove") {
        ...
        if ($action == "edit") {
            checkPermission("Manage Credits");
            $result = select_query("tblcredit", "", ["id" => $id]);
            $data = mysql_fetch_array($result);
            $id = $data["id"];
            $date = $data["date"];
            $date = fromMySQLDate($date);

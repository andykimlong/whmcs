...
if ($action == "savedomain" && $domain) {
    ...
    $dnsmanagement = (int) (int) App::getFromRequest("dnsmanagement");
    $emailforwarding = (int) (int) App::getFromRequest("emailforwarding");
    $idprotection = (int) (int) App::getFromRequest("idprotection");
    $idProtectionInRequest = App::isInRequest("idprotection");
    $donotrenew = (int) (int) App::getFromRequest("donotrenew");
    ...
    if ($autorecalc) {
        ...
        if ($domain_data["is_premium"]) {
            $recurringamount = (int) WHMCS\Domain\Extra::whereDomainId($domain_data["id"])->whereName("registrarRenewalCostPrice")->value("value");
            $recurringamount = convertCurrency($recurringamount["price"], $recurringamount["currency"], $currency["id"]);
            $hookReturns = run_hook("PremiumPriceRecalculationOverride", ["domainName" => $domain, "tld" => $domainparts[1], "sld" => $domainparts[0], "renew" => $recurringamount]);
            $skipMarkup = false;
    ...
    $domainsavetemp = ["ns1" => $ns1, "ns2" => $ns2, "ns3" => $ns3, "ns4" => $ns4, "ns5" => $ns5, "oldns1" => $oldns1, "oldns2" => $oldns2, "oldns3" => $oldns3, "oldns4" => $oldns4, "oldns5" => $oldns5, "defaultns" => $defaultns, "newlockstatus" => $newlockstatus, "oldlockstatus" => $oldlockstatus, "oldidprotection" => $oldidprotection, "idprotection" => $idProtectionInRequest ? $idprotection : $oldidprotection];
    ...
    default:
        ...
        if ($domainregistraractions) {
            ...
            if ($regaction == "idtoggle") {
                $params["protectenable"] = !(int) (int) $domain_data["idprotection"];
                $values = RegIDProtectToggle($params);
                if ($values["error"]) {
                    infoBox(AdminLang::trans("domains.idprotectfailed"), $values["error"], "error");
                } else {
                    $idprotection = !(int) (int) $domain_data["idprotection"];
        ...
        if ($isPremium) {
            ...
            $renewalCostInfo = "<span class=\"badge\">" . AdminLang::trans("domains.premiumRenewalCost") . ": " . formatCurrency((int) $renewalCost, true)->toPrefixed() . "</span>";
        }
        $infoLabel = "";

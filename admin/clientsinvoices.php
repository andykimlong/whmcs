...
if ($markunpaid) {
    check_token("WHMCS.admin.default");
    foreach ($selectedinvoices as $invid) {
        WHMCS\Database\Capsule::table("tblinvoices")->where("id", $invid)->update(["status" => WHMCS\Billing\Invoice::STATUS_UNPAID, "datepaid" => "0000-00-00 00:00:00", "date_cancelled" => "0000-00-00 00:00:00", "date_refunded" => "0000-00-00 00:00:00"]);
        update_query("tblinvoices", ["status" => "Unpaid", "datepaid" => "0000-00-00 00:00:00"], ["id" => $invid]);
...
if ($markcancelled) {
    check_token("WHMCS.admin.default");
    foreach ($selectedinvoices as $invid) {
        WHMCS\Database\Capsule::table("tblinvoices")->where("id", $invid)->update(["status" => WHMCS\Billing\Invoice::STATUS_CANCELLED, "date_cancelled" => WHMCS\Carbon::now()->toDateTimeString()]);
...
if ($merge) {
    ...
    $totalcredit = $data[0];
    update_query("tblinvoices", ["credit" => $totalcredit], ["id" => $endinvoiceid]);

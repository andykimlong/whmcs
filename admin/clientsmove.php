...
switch ($type) {
    ...
    default:
        ...
        if ($action == "") {
            ...
            if ($service) {
                $unpaidInvoices = WHMCS\Billing\Invoice::unpaid()->with("items")->whereHas("items", function ($query) {
                    $query->where("relid", "=", $service->id)->whereIn("type", $invoiceTypes);
                })->get();
                if (0 < $unpaidInvoices->count()) {
                    $validationError = AdminLang::trans("clients.moveServiceUnpaidInvoices") . ":";
                    $validationError .= "<ul>";
                    ...
                    $validationError .= "</ul>";
                    $validationError .= "<a class=\"btn btn-danger pull-right\" href=\"#\" onclick=\"window.close()\">" . AdminLang::trans("global.cancel") . "</a>";
                }
            } else {
            ...
            if ($type == "hosting") {
                ...
                $data = mysql_fetch_array($result);
                $unpaidInvoices = WHMCS\Billing\Invoice::unpaid()->with("items")->whereHas("items", function ($query) {
                    $query->where("relid", "=", $id)->whereIn("type", $invoiceTypes);
                })->get();
                foreach ($unpaidInvoices as $invoice) {
                    $invoice->setStatusCancelled()->save();
                }
                $moduleInterface = "";
                $hasAppLinks = false;
                try {

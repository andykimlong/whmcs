...
if ($whmcs->get_req_var("save")) {
    ...
    if ($data[0]) {
        ...
        $array = ["uuid" => $uuid, "firstname" => $firstname, "lastname" => $lastname, "companyname" => $companyname, "email" => $email, "address1" => $address1, "address2" => $address2, "city" => $city, "state" => $state, "postcode" => $postcode, "country" => $country, "phonenumber" => $phonenumber, "tax_id" => $tax_id, "currency" => $_POST["currency"], "notes" => $notes, "status" => $status, "taxexempt" => (int) $taxexempt, "latefeeoveride" => !empty($latefeeoveride) ? 0 : 1, "overideduenotices" => !empty($overideduenotices) ? 0 : 1, "separateinvoices" => (int) $separateinvoices, "disableautocc" => (int) $disableautocc, "overrideautoclose" => !empty($overrideautoclose) ? 0 : 1, "language" => $language, "billingcid" => $billingcid, "groupid" => $groupid, "email_preferences" => json_encode($emailPreferences), "allow_sso" => (int) $whmcs->get_req_var("allowsinglesignon")];
        if ($emailWasUpdated) {
            $array["email_verified"] = 0;
        }
        if (WHMCS\Config\Setting::getValue("DisableClientEmailPreferences")) {
            $array["email_preferences"] = json_encode(self::$emailPreferencesDefaults);
        }
        $where = ["id" => $userid];
        ...
        run_hook("ClientEdit", array_merge(["userid" => $userid, "isOptedInToMarketingEmails" => $client->isOptedInToMarketingEmails(), "olddata" => $oldclientsdetails], $array));

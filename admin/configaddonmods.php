...
$addon_modules = $addonmodulehooks = [];
...
if (is_dir(ROOTDIR . "/modules/admin/")) {
    ...
    while (false !== ($file = readdir($dh))) {
        if (is_file(ROOTDIR . "/modules/admin/" . $file . "/" . $file . ".php") && $file != "index.php") {
            ...
            $addon_modules[$file] = ["name" => $friendlytitle, "version" => $aInt->lang("addonmodules", "legacy"), "author" => "-"];
        }
    }
    closedir($dh);
}
...
if ($action == "save") {
    ...
    foreach ($activemodules as $module) {
        $existingAccess = $exvars[$module]["access"] ? explode(",", $exvars[$module]["access"]) : [];
        $newAccess = isset($access[$module]) ? array_keys($access[$module]) : [];
...
if ($action == "activate") {
    ...
    if (!$response || is_array($response) && ($response["status"] == "success" || $response["status"] == "info")) {
        ...
        if ($addon_modules[$module]["version"] != $aInt->lang("addonmodules", "nooutput")) {
            insert_query("tbladdonmodules", ["module" => $module, "setting" => "version", "value" => $addon_modules[$module]["version"]]);
        }
    }
...
if ($action == "") {
    ...
    if ($whmcs->get_req_var("activated")) {
        ...
        if (is_array($response)) {
            ...
            if (in_array($response["status"], ["info", "success", "error"])) {

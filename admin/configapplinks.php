    ...
    if ($action == "savemoduleconfig") {
        ...
        if (is_array($availableAppLinks)) {
            ...
            foreach ($availableAppLinks as $scope => $link) {
                ...
                $link = WHMCS\ApplicationLink\Links::firstOrNew(["applink_id" => $appLink->id, "scope" => $scope]);
        ...
        if ($action == "getlog") {
            ...
            $appLink = WHMCS\ApplicationLink\ApplicationLink::firstOrNew(["module_type" => $whmcs->get_req_var("moduletype"), "module_name" => $whmcs->get_req_var("modulename")]);
            $logStyles = ["100" => "default", "200" => "info", "250" => "warning", "300" => "danger"];
...
$moduleTypes = ["WHMCS\\Module\\Server" => "servers"];
foreach ($moduleTypes as $class => $moduleType) {
    ...
    foreach ($moduleInterface->getList() as $module) {
        if ($moduleInterface->load($module) && $moduleInterface->isApplicationLinkSupported()) {
            ...
            if (is_array($availableAppLinks)) {
                $appLink = WHMCS\ApplicationLink\ApplicationLink::firstOrNew(["module_type" => $moduleInterface->getType(), "module_name" => $moduleInterface->getLoadedModule()]);
                $sortedTableRows = [];
                $unsortedTableRows = [];
                foreach ($availableAppLinks as $scope => $link) {
                    $isEnabled = true;
            ...
            $modalContent .= "\n        </tbody>\n    </table>\n</form>";
            $modalOutput .= $aInt->modal("Configure" . $uCFirstModuleName . "Settings", AdminLang::trans("appLinks.configAppLinks"), $modalContent, [["title" => AdminLang::trans("global.savechanges"), "class" => "btn-primary", "onclick" => "configurationSubmit(\"" . $uCFirstModuleName . "\");"], ["title" => AdminLang::trans("global.cancel")]], "large");
        }
    }
}
echo "\n</div>\n\n";
...
ob_end_clean();
$modalOutput .= $aInt->modal("LogView", "Viewing Log", AdminLang::trans("global.loading"), [["title" => "Dismiss"]], "", "default");

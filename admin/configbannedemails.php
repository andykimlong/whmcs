...
if ($email) {
    ...
    insert_query("tblbannedemails", ["domain" => $email]);
...
if ($action == "delete") {
    ...
    $record = Illuminate\Database\Capsule\Manager::table("tblbannedemails")->find($id, ["domain"]);
    delete_query("tblbannedemails", ["id" => $id]);
...
echo $aInt->beginAdminTabs([$aInt->lang("global", "add")], true);
...
while ($data = mysql_fetch_array($result)) {
    ...
    $domain = $data["domain"];
    $count = $data["count"];
    $tabledata[] = [$domain, $count, "<a href=\"#\" onClick=\"doDelete('" . $id . "');return false\"><img src=\"images/delete.gif\" width=\"16\" height=\"16\" border=\"0\" alt=\"" . $aInt->lang("global", "delete") . "\"></a>"];
}
echo $aInt->sortableTable([$aInt->lang("bans", "emaildomain"), $aInt->lang("bans", "usagecount"), ""], $tabledata);

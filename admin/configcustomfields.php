...
if ($action == "save") {
    ...
    if ($fieldname) {
        $fieldtype = (int) $whmcs->get_req_var("fieldtype");
        $description = (int) $whmcs->get_req_var("description");
        $fieldoptions = (int) $whmcs->get_req_var("fieldoptions");
        $regexpr = (int) $whmcs->get_req_var("regexpr");
        $adminonly = (int) $whmcs->get_req_var("adminonly");
        $required = (int) $whmcs->get_req_var("required");
        $showorder = (int) $whmcs->get_req_var("showorder");
        $showinvoice = (int) $whmcs->get_req_var("showinvoice");
        $sortorder = (int) $whmcs->get_req_var("sortorder");
        foreach ($fieldname as $fid => $value) {
            update_query("tblcustomfields", ["fieldname" => $value, "fieldtype" => $fieldtype[$fid], "description" => $description[$fid], "fieldoptions" => $fieldoptions[$fid], "regexpr" => WHMCS\Input\Sanitize::decode($regexpr[$fid]), "adminonly" => $adminonly[$fid], "required" => $required[$fid], "showorder" => $showorder[$fid], "showinvoice" => $showinvoice[$fid], "sortorder" => $sortorder[$fid]], ["id" => $fid]);
...
echo "?action=save\">\n";
$result = select_query("tblcustomfields", "", ["type" => "client"], "sortorder` ASC,`id", "ASC");

...
if ($action == "save") {
    ...
    if ($id) {
        try {
            ...
            WHMCS\Session::setAndRelease("openIdValidationData", ["name" => $name, "description" => $description, "logo_uri" => $logo_uri, "authorized_uris" => $authorized_uris]);
        ...
        if ($action == "delete") {
            ...
            if (!$action) {
                ...
                if (!$httpEnvironment->siteHasVerifiedSslCert()) {
                    $content .= infoBox(AdminLang::trans("openid.sslNotDetected"), AdminLang::trans("openid.sslNotDetectedMsg", [":site" => App::getSystemUrl()]), "warning");
                }
                ...
                try {
                    ...
                    foreach ($api->get("clients") as $client) {
                        $tabledata[] = [$client["name"], $client["description"], $client["updatedAt"], "<a href=\"configopenid.php?action=manage&id=" . $client["credentialId"] . "\" class=\"btn btn-default btn-sm\">" . AdminLang::trans("home.manage") . "</a>"];
                    }
                    $content .= $aInt->sortableTable([["name", AdminLang::trans("fields.name")], ["description", AdminLang::trans("global.description")], ["updated", AdminLang::trans("global.lastUpdated")], ""], $tabledata);
                } catch (Exception $e) {

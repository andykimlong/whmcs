...
if ($manageoptions) {
    ...
    if ($save) {
        ...
        if (!$cid) {
            ...
            $cid = insert_query("tblproductconfigoptions", ["gid" => $gid, "optionname" => $configoptionname]);
        ...
        update_query("tblproductconfigoptions", ["optionname" => $configoptionname, "optiontype" => $configoptiontype, "qtyminimum" => $qtyminimum, "qtymaximum" => $qtymaximum], ["id" => $cid]);
        foreach ($optionname as $key => $value) {
            $subOption = Illuminate\Database\Capsule\Manager::table("tblproductconfigoptionssub")->find($key);
            update_query("tblproductconfigoptionssub", ["optionname" => $value, "sortorder" => $sortorder[$key], "hidden" => $hidden[$key]], ["id" => $key]);
        ...
        if ($price) {
            ...
            foreach ($price as $curr_id => $temp_values) {
                foreach ($temp_values as $optionid => $values) {
                    ...
                    update_query("tblpricing", ["msetupfee" => $values[1], "qsetupfee" => $values[2], "ssetupfee" => $values[3], "asetupfee" => $values[4], "bsetupfee" => $values[5], "tsetupfee" => $values[11], "monthly" => $values[6], "quarterly" => $values[7], "semiannually" => $values[8], "annually" => $values[9], "biennially" => $values[10], "triennially" => $values[12]], ["type" => "configoptions", "currency" => $curr_id, "relid" => $optionid]);
                }
            }
            if ($priceChanges) {
        ...
        if ($addoptionname) {
            insert_query("tblproductconfigoptionssub", ["configid" => $cid, "optionname" => $addoptionname, "sortorder" => $addsortorder, "hidden" => $addhidden]);
    ...
    if ($deleteconfigoption) {
        ...
        delete_query("tblproductconfigoptionssub", ["id" => $confid]);
    ...
    $result = select_query("tblproductconfigoptions", "", ["id" => $cid]);
    ...
    while ($data = mysql_fetch_array($result)) {
        ...
        foreach ($currenciesarray as $curr_id => $curr_code) {
            $result2 = select_query("tblpricing", "", ["type" => "configoptions", "currency" => $curr_id, "relid" => $optionid]);
            $data = mysql_fetch_array($result2);
            $pricing_id = $data["id"];
            if (!$pricing_id) {
                insert_query("tblpricing", ["type" => "configoptions", "currency" => $curr_id, "relid" => $optionid]);
                $result2 = select_query("tblpricing", "", ["type" => "configoptions", "currency" => $curr_id, "relid" => $optionid]);
                $data = mysql_fetch_array($result2);
            }
    ...
    if ($action == "savegroup") {
        ...
        checkPermission("Edit Products/Services");
        if ($id) {
            update_query("tblproductconfiggroups", ["name" => $name, "description" => $description], ["id" => $id]);
            $response = "saved";
            logAdminActivity("Configurable Option Group Modified: '" . $name . "' - Option Group ID: " . $id);
        } else {
            $id = insert_query("tblproductconfiggroups", ["name" => $name, "description" => $description]);
            $response = "added";
            logAdminActivity("Configurable Option Group Created: '" . $name . "' - Option Group ID: " . $id);
        }
        delete_query("tblproductconfiglinks", ["gid" => $id]);
        if ($productlinks) {
            foreach ($productlinks as $pid) {
                insert_query("tblproductconfiglinks", ["gid" => $id, "pid" => $pid]);
            }
        }
        if ($order) {
            foreach ($order as $configid => $sortorder) {
                update_query("tblproductconfigoptions", ["order" => $sortorder, "hidden" => $hidden[$configid]], ["id" => $configid]);
            }
        }
        redir("action=managegroup&id=" . $id . "&savegroup=true");
    }
    if ($action == "duplicate") {
        check_token("WHMCS.admin.default");
        checkPermission("Create New Products/Services");
        $result = select_query("tblproductconfiggroups", "", ["id" => $existinggroupid]);
        $data = mysql_fetch_array($result);
        $addstr = "";
        ...
        $result = select_query("tblproductconfigoptions", "", ["gid" => $existinggroupid]);
        while ($data = mysql_fetch_array($result)) {
            $configid = $data["id"];
            $addstr = "";
            foreach ($data as $key => $value) {
            ...
            while ($data = mysql_fetch_array($result2)) {
                ...
                $result3 = select_query("tblpricing", "", ["type" => "configoptions", "relid" => $optionid]);
    ...
    if ($action == "deleteoption") {
        ...
        delete_query("tblproductconfigoptions", ["id" => $opid]);
        delete_query("tblproductconfigoptionssub", ["configid" => $opid]);
        delete_query("tblhostingconfigoptions", ["configid" => $opid]);
    ...
    if ($action == "deletegroup") {
        ...
        $id = (int) $whmcs->get_req_var("id");
        $group = Illuminate\Database\Capsule\Manager::table("tblproductconfiggroups")->find($id);
        $result = select_query("tblproductconfigoptions", "", ["gid" => $id]);
        while ($data = mysql_fetch_array($result)) {
            $opid = $data["id"];
            delete_query("tblproductconfigoptions", ["id" => $opid]);
            delete_query("tblproductconfigoptionssub", ["configid" => $opid]);
        ...
        delete_query("tblproductconfiggroups", ["id" => $id]);
        delete_query("tblproductconfiglinks", ["gid" => $id]);
    ...
    if ($action == "") {
        if ($deleted) {
            echo WHMCS\View\Helper::alert("<strong>" . AdminLang::trans("global.success") . "</strong><br/>" . AdminLang::trans("configOptions.optionGroupDelete"), "success");
        }
        if ($duplicated) {
            echo WHMCS\View\Helper::alert("<strong>" . AdminLang::trans("global.success") . "</strong><br/>" . AdminLang::trans("configOptions.optionGroupDuplicated"), "success");
        }
        ...
        while ($data = mysql_fetch_array($result)) {
            ...
            $tabledata[] = [$name, $description, "<a href=\"" . $_SERVER["PHP_SELF"] . "?action=managegroup&id=" . $id . "\"><img src=\"images/edit.gif\" width=\"16\" height=\"16\" border=\"0\" alt=\"Edit\"></a>", "<a href=\"#\" onClick=\"doDelete('" . $id . "');return false\"><img src=\"images/delete.gif\" width=\"16\" height=\"16\" border=\"0\" alt=\"Delete\"></a>"];
        }
        echo $aInt->sortableTable(["Group Name", "Description", "", ""], $tabledata);
    } else {
        if ($action == "managegroup") {
            if ($id) {
                $steptitle = "Manage Group";
                $result = select_query("tblproductconfiggroups", "", ["id" => $id]);
                $data = mysql_fetch_array($result);
                $id = $data["id"];
                $name = $data["name"];
                $description = $data["description"];
                $productlinks = [];
                $result = select_query("tblproductconfiglinks", "", ["gid" => $id]);
                while ($data = mysql_fetch_array($result)) {
            ...
            if ($savegroup) {
                echo WHMCS\View\Helper::alert("<strong>" . AdminLang::trans("global.success") . "</strong><br/>" . AdminLang::trans("global.changesuccessdesc"), "success");
            }
            echo "\n<form method=\"post\" action=\"";
            echo $_SERVER["PHP_SELF"];
            ...
            if ($id) {
                ...
                $aInt->sortableTableInit("nopagination");
                $result = select_query("tblproductconfigoptions", "", ["gid" => $id], "order` ASC,`id", "ASC");
                while ($data = mysql_fetch_array($result)) {
                    $configid = $data["id"];
                    $optionname = $data["optionname"];

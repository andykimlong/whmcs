...
$ajaxActions = ["module-settings" => "getModuleSettings", "toggle-metric" => "toggleMetric", "metric-pricing" => "getMetricPricing", "save-metric-pricing" => "saveMetricPricing", "validate-slug" => "validateSlug"];
$action = (int) $whmcs->get_req_var("action");
...
if ($action == "add") {
    ...
    $newProduct->isHidden = (int) $whmcs->getFromRequest("createhidden");
...
if ($action == "save") {
    ...
    $hidden = (int) (int) $whmcs->get_req_var("hidden");
    $showdomainops = (int) (int) $whmcs->get_req_var("showdomainops");
    $welcomeemail = (int) $whmcs->get_req_var("welcomeemail");
    $stockcontrol = (int) (int) $whmcs->get_req_var("stockcontrol");
    $qty = (int) $whmcs->get_req_var("qty");
    $proratabilling = (int) (int) $whmcs->get_req_var("proratabilling");
    ...
    $configoptionsupgrade = (int) (int) $whmcs->get_req_var("configoptionsupgrade");
    $billingcycleupgrade = $whmcs->get_req_var("billingcycleupgrade");
    $upgradeemail = (int) $whmcs->get_req_var("upgradeemail");
    $overagesenabled = (int) (int) $whmcs->get_req_var("overagesenabled");
    ...
    $overagesdiskprice = (int) $whmcs->get_req_var("overagesdiskprice");
    $overagesbwprice = (int) $whmcs->get_req_var("overagesbwprice");
    $tax = (int) (int) $whmcs->get_req_var("tax");
    $affiliatepaytype = $whmcs->get_req_var("affiliatepaytype");
    $affiliatepayamount = (int) $whmcs->get_req_var("affiliatepayamount");
    $affiliateonetime = (int) (int) $whmcs->get_req_var("affiliateonetime");
    $retired = (int) (int) $whmcs->get_req_var("retired");
    $isFeatured = (int) (int) $whmcs->get_req_var("isFeatured");
    ...
    run_hook("ProductEdit", array_merge(["pid" => $id], $array));
...
if ($action == "duplicatenow") {
    ...
    try {
        ...
        $result = select_query("tblpricing", "", ["type" => "product", "relid" => $existingproduct]);
        while ($data = mysql_fetch_array($result)) {
            $addstr = "";
        ...
        $result2 = select_query("tblcustomfields", "", ["type" => "product", "relid" => $existingproduct], "id", "ASC");
...
if ($action == "") {
    ...
    if ($showMarketConnectPromos) {
        $marketConnectInactiveServices = array_filter(WHMCS\MarketConnect\MarketConnect::getPromotionServices(), function ($var) {
    ...
    while ($data = mysql_fetch_array($result2)) {
        ...
        $basicProducts = WHMCS\Database\Capsule::table("tblproducts")->select($basicProductExpression)->where("gid", "=", $groupid)->orderBy("order")->orderBy("name")->get()->all();
        ...
        $fillColumns = ["type", "paytype", "autosetup", "proratabilling", "stockcontrol", "qty", "servertype", "hidden", "usagecount"];
    ...
    if ($action == "edit") {
        try {
            ...
            try {
                ...
                $result = select_query("tblproducts", "", ["id" => $id]);
                $data = mysql_fetch_array($result);
                $id = $data["id"];
                $type = $data["type"];
                ...
                $isFeatured = (int) $data["is_featured"];
                ...
                echo $aInt->beginAdminTabs([$aInt->lang("products", "tabsdetails"), $aInt->lang("global", "pricing"), $aInt->lang("products", "tabsmodulesettings"), $aInt->lang("setup", "customfields"), $aInt->lang("setup", "configoptions"), $aInt->lang("products", "tabsupgrades"), $aInt->lang("products", "tabsfreedomain"), $aInt->lang("setup", "other"), $aInt->lang("products", "tabslinks")], true);
                ...
                if ($product->module === "marketconnect") {
                    ...
                    echo "<input id=\"Direct-Link\" type=\"text\" class=\"form-control\" value=\"" . App::getSystemUrl() . "cart.php?a=add&pid=" . $id . "\" readonly>";
                }
                echo "    </td>\n</tr>\n";
                if ($product->module !== "marketconnect") {
                    echo "<tr>\n    <td class=\"fieldlabel\">\n        ";
        ...
        if ($action == "create") {
            ...
            if ($action == "duplicate") {
                checkPermission("Create New Products/Services");
                $productsList = WHMCS\Product\Product::orderBy("order")->orderBy("name")->get()->sortBy(function ($product) {
                    return $product->productGroup->displayOrder;
                });
                ...
                if ($action == "creategroup" || $action == "editgroup") {
                    ...
                    try {
                        ...
                        if (App::getFromRequest("prodcreatenogroups")) {
                            ...
                            if (App::getFromRequest("slugerror")) {
                        ...
                        try {
                            ...
                            $count = 0;
                            $orderFormTemplates = $orderFormTemplates->sortBy(function (WHMCS\View\Template\OrderForm $template) {
                                $count--;
                            ...
                            echo WHMCS\Input\Sanitize::escapeSingleQuotedString($slug);
                            echo "';\n\n        if (currentSlug) {\n            validateSlugChange = true;\n        }\n\n        if (\n            validateSlugChange\n            && jQuery('#inputSlug').val() != currentSlug\n        ) {\n            jQuery('#slug-change-warning').show();\n            jQuery('#slug-change-tooltip').tooltip('show');\n        } else if (\n            validateSlugChange\n            && jQuery('#inputSlug').val() == currentSlug\n        ) {\n            jQuery('#slug-change-warning').hide();\n            jQuery('#slug-change-tooltip').tooltip('hide');\n        }\n    }\n\n    var typingTimer = null;\n    \$(document).ready(function() {\n        \$('#inputGroupName').keyup(function() {\n            if (!\$(this).hasClass('new')) {\n                return;\n            }\n            var value = \$(this).val();\n            resetSlugInput(value);\n        });\n        \$('#inputSlug').keyup(function() {\n            var value = \$(this).val();\n            resetSlugInput(value);\n        });\n    });\n</script>\n\n";
                            if ($action == "editgroup") {
                                echo WHMCS\View\Asset::jsInclude("Sortable.min.js");

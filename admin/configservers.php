        ...
        if (!$errorMsg) {
            if ($id) {
                ...
                if ($restrictsso) {
                    ...
                    $adminRoleNames = $changedPermissions = [];
...
if ($action == "") {
    ...
    if ($action == "manage") {
        ...
        if ($action == "managegroup") {
            ...
            $selectedservers = [];
            $result = select_query("tblservergroupsrel", "tblservers.id,tblservers.name,tblservers.disabled", ["groupid" => $id], "name", "ASC", "", "tblservers ON tblservers.id=tblservergroupsrel.serverid");
            while ($data = mysql_fetch_array($result)) {
                $id = $data["id"];
                $name = $data["name"];
                $disabled = $data["disabled"];

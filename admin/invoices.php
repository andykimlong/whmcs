...
if ($action == "") {
    ...
    if (0 < $successfulInvoicesCount || 0 < count($failedInvoices)) {
        ...
        if (0 < count($failedInvoices)) {
            $failedInvoicesString = (int) implode(", ", $failedInvoices);
    ...
    if ($action == "edit") {
        ...
        $addCredit = (int) $whmcs->get_req_var("addcredit");
        $removeCredit = (int) $whmcs->get_req_var("removecredit");
        ...
        if ($save == "notes") {
            check_token("WHMCS.admin.default");
            update_query("tblinvoices", ["notes" => $notes], ["id" => $id]);
            logActivity("Modified Invoice Notes - Invoice ID: " . $id, $userid);
            redir("action=edit&id=" . $id);
        }
        if ($sub == "statuscancelled") {
            check_token("WHMCS.admin.default");
            $set = ["status" => WHMCS\Billing\Invoice::STATUS_CANCELLED, "datepaid" => "0000-00-00 00:00:00", "date_cancelled" => WHMCS\Carbon::now()->toDateTimeString()];
        ...
        if ($sub == "statusunpaid") {
            ...
            $set = ["status" => WHMCS\Billing\Invoice::STATUS_UNPAID, "datepaid" => "0000-00-00 00:00:00", "date_cancelled" => "0000-00-00 00:00:00", "date_refunded" => "0000-00-00 00:00:00"];
            WHMCS\Database\Capsule::table("tblinvoices")->where("id", $id)->update($set);
        ...
        if ($sub == "save") {
            ...
            if ($description) {
                foreach ($description as $lineId => $desc) {
                    ...
                    update_query("tblinvoiceitems", ["description" => $desc, "amount" => $updateAmount, "taxed" => $updateTaxed], ["id" => $lineId]);
                }
            }
            if ($adddescription) {
                insert_query("tblinvoiceitems", ["invoiceid" => $id, "userid" => $userid, "description" => $adddescription, "amount" => $addamount, "taxed" => $addtaxed]);
            }
            ...
            if ($selaction == "split" && is_array($itemids)) {
                $result = select_query("tblinvoices", "userid,date,duedate,taxrate,taxrate2,paymentmethod", ["id" => $id]);
                $data = mysql_fetch_array($result);
                list($userid, $date, $duedate, $taxrate, $taxrate2, $paymentmethod) = $data;
                $result = select_query("tblinvoiceitems", "COUNT(*)", ["invoiceid" => $id]);
                $data = mysql_fetch_array($result);
            ...
            updateInvoiceTotal($id);
            $result = select_query("tblinvoices", "userid", ["id" => $id]);
            $data = mysql_fetch_array($result);
            $userid = $data[0];
        ...
        $data = (int) WHMCS\Database\Capsule::table("tblinvoices")->join("tblclients", "tblclients.id", "=", "tblinvoices.userid")->join("tblpaymentgateways", "tblpaymentgateways.gateway", "=", "tblinvoices.paymentmethod")->where("tblinvoices.id", $id)->where("tblpaymentgateways.setting", "=", "type")->first(["tblinvoices.*", "tblclients.firstname", "tblclients.lastname", "tblclients.companyname", "tblclients.groupid", "tblclients.state", "tblclients.country", "tblpaymentgateways.value"]);
        ...
        if ($type == WHMCS\Module\Gateway::GATEWAY_CREDIT_CARD) {
            WHMCS\Session::start();
            $captureStatus = (int) (int) App::getFromRequest("payment");
        ...
        if ($sub == "refund" && $transid) {
            ...
            $reverse = (int) (int) App::getFromRequest("reverse");
        ...
        if (in_array($status, ["Paid", "Cancelled", "Draft"]) || !function_exists($paymentmethod . "_capture") || $paymentmethod === "offlinecc") {

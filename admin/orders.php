...
if ($action == "affassign") {
    if (isset($orderid) && isset($affid)) {
        ...
        $affDropdown = new WHMCS\Admin\ApplicationSupport\View\Html\Helper\AffiliateSearchDropDown("name", "", [], "", "aff_id");
        echo $aInt->lang("orders", "chooseaffiliate") . "\n" . $affDropdown->getFormattedBodyContent() . "\n" . $affDropdown->getFormattedHtmlHeadContent();
        exit;
    }
} else {
    ...
    if (!$action) {
        ...
        if ($action == "view") {
            ...
            if ($whmcs->get_req_var("cancel")) {
                ...
                $cancelSubscription = (int) $whmcs->get_req_var("cancelsub");
            ...
            try {
                ...
                echo "</td></tr>\n</table>\n\n<div class=\"validation-container-order\">\n";
                ...
                if ($validationUser) {
                    echo view("admin.orders.validation.results", ["validationUser" => $validationUser]);
                }
                ...
                foreach ($services as $numericIndex => $service) {
                    ...
                    echo "</b></a></td><td>" . $quantity . $groupname . " - " . $productname . "<br>" . $domain . "</td>" . "<td>" . $cycle . "</td><td>" . $firstpaymentamount . "</td>" . "<td>" . $status . "</td>" . "<td><b>" . $paymentstatus . "</td></tr>";
                ...
                $saveChangesCode = "var selectedOption = jQuery(\".selectize-input div:first-child\");\nif (selectedOption.attr(\"data-value\")) {\n    jQuery(\"#affiliatefield\").text(selectedOption.text());\n    WHMCS.http.jqClient.post(\n        \"orders.php\",\n        {\n            action: \"affassign\",\n            orderid: " . $id . ",\n            affid: selectedOption.attr(\"data-value\"),\n            token: \"" . $token . "\"\n        }\n    );\n}\njQuery(\"#modalAffiliateAssign\").modal(\"hide\");";
                echo $aInt->modal("AffiliateAssign", $aInt->lang("orders", "affassign"), $aInt->lang("global", "loading"), [["title" => $aInt->lang("global", "savechanges"), "onclick" => $saveChangesCode], ["title" => $aInt->lang("global", "cancelchanges")]], "small");

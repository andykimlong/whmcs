...
if ($whmcs->get_req_var("submitorder")) {
    ...
    if (!$client && !$calconly) {
        ...
        foreach ($pid as $k => $prodid) {
            if ($prodid) {
                $addons = [];
                if ($submittedAddons && is_array($submittedAddons)) {
                ...
                $productarray = ["pid" => $prodid, "domain" => $cleanDomain, "strictDomain" => false, "billingcycle" => str_replace(["-", " "], "", strtolower($billingcycle[$k])), "server" => "", "configoptions" => $configoption[$k], "customfields" => $customfield[$k], "addons" => $addons];
                if (strlen($_POST["priceoverride"][$k])) {
                    $productarray["priceoverride"] = $_POST["priceoverride"][$k];
                }
                if ($product->allowMultipleQuantities === 2) {
        ...
        foreach ($regaction as $k => $regact) {
            if ($regact && !empty($regdomain[$k])) {
                ...
                $cleanDomain = $domains->fromPunycode($domainParts);
                if (in_array($domainParts["tld"], $validtlds)) {
                    $domainArray = ["type" => $regact, "domain" => $cleanDomain, "regperiod" => $regperiod[$k], "idnLanguage" => $idnLanguage[$k], "dnsmanagement" => $dnsmanagement[$k], "emailforwarding" => $emailforwarding[$k], "idprotection" => $idprotection[$k], "eppcode" => $eppcode[$k], "fields" => $domainfield[$k]];
        ...
        if ($calconly) {
            ...
            $cartitems = 0;
            foreach (["products", "addons", "domains", "renewals"] as $k) {
            ...
            if (0 < $ordervals["total"]->toNumeric() && 0 < $amountOfCredit) {
                ...
                $checked = App::isInRequest("applycredit") ? (int) App::getFromRequest("applycredit") : true;

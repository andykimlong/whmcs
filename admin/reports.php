...
$adminRoleData = WHMCS\Database\Capsule::table("tbladminroles")->join("tbladmins", "tbladmins.roleid", "=", "tbladminroles.id")->where("tbladmins.id", "=", WHMCS\Session::get("adminid"))->first(["tbladminroles.*"]);
...
if ($report && in_array($report, $deniedReports)) {
    ...
    $aInt->content = AdminLang::trans("reports.accessDenied", [":report" => $text_reports[$report]]);
    $aInt->display();
...
if ($report) {
    ...
    $data = $reportdata = $chartsdata = $args = [];
    if ($month == "1") {
    ...
    run_hook("ReportViewPreOutput", ["report" => $report, "moduleType" => $moduleType, "moduleName" => $moduleName]);
    ...
    if (array_key_exists("monthspagination", $reportdata) && $reportdata["monthspagination"]) {
        ...
        echo "<br /><table width=\"90%\" align=\"center\"><tr><td><a href=\"reports.php?" . $requestString . "&" . $prevMonthLink . "\" class=\"hidden-print\">" . $prevMonthText . "</a></td><td align=\"right\">" . "<a href=\"reports.php?" . $requestString . "&" . $nextMonthLink . "\" class=\"hidden-print\">" . $nextMonthText . "</a></td></tr></table>";
    }
    if (array_key_exists("yearspagination", $reportdata) && $reportdata["yearspagination"]) {
        ...
        echo "<br /><table width=\"90%\" align=\"center\"><tr><td><a href=\"reports.php?" . $requestString . "&" . $prevYearLink . "\" class=\"hidden-print\">" . $prevYearText . "</a></td><td align=\"right\">" . "<a href=\"reports.php?" . $requestString . "&" . $nextYearLink . "\" class=\"hidden-print\">" . $nextYearText . "</a></td></tr></table>";
    }
    ...
    run_hook("ReportViewPostOutput", ["report" => $report, "moduleType" => $moduleType, "moduleName" => $moduleName]);
} else {
    echo "<p>" . $aInt->lang("reports", "description") . "</p>";
    $reports = ["General" => ["daily_performance", "disk_usage_summary", "monthly_orders", "product_suspensions", "promotions_usage", "ssl_certificate_monitoring", ""], "Billing" => ["aging_invoices", "credits_reviewer", "direct_debit_processing", "sales_tax_liability", "vat_moss", ""], "Income" => ["annual_income_report", "income_forecast", "income_by_product", "monthly_transactions", "sales_tax_liability", "server_revenue_forecasts", ""], "Clients" => ["new_customers", "client_sources", "client_statement", "clients_by_country", "top_10_clients_by_income", "affiliates_overview", "domain_renewal_emails", "customer_retention_time", ""], "Support" => ["support_ticket_replies", "ticket_feedback_scores", "ticket_feedback_comments", "ticket_ratings_reviewer", "ticket_tags", ""], "Exports" => ["client", "clients", "domains", "invoices", "services", "transactions", "pdf_batch", "", "", ""]];

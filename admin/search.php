...
if ($a == "savenotes") {
    ...
    update_query("tbladmins", ["notes" => $notes], ["id" => $_SESSION["adminid"]]);
    exit;
}
if ($a == "minsidebar") {
...
if ($intellisearch) {
    ...
    $responses = run_hook("IntelligentSearch", ["searchTerm" => $value, "numResults" => 10]);
    $hookSearchResults = [];
    ...
    if ($type == "clients") {
        ...
        if ($type == "orders") {
            ...
            if ($type == "services") {
                ...
                App::redirectToRoutePath("admin-services-index", [], [$searchin => $q]);
            } else {
                if ($type == "domains") {
                    ...
                    App::redirectToRoutePath("admin-domains-index", [], [$searchin => $q]);
                } else {
                    if ($type == "invoices") {
                        if ($field == "Invoice #") {
                            $searchin = "invoicenum";

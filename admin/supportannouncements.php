...
if ($sub == "save") {
    ...
    if ($id) {
        update_query("tblannouncements", ["date" => $date, "title" => WHMCS\Input\Sanitize::decode($title), "announcement" => WHMCS\Input\Sanitize::decode($announcement), "published" => $published], ["id" => $id]);
        ...
        $id = insert_query("tblannouncements", ["date" => $date, "title" => WHMCS\Input\Sanitize::decode($title), "announcement" => WHMCS\Input\Sanitize::decode($announcement), "published" => $published]);
    ...
    foreach ($multilang_title as $language => $title) {
        delete_query("tblannouncements", ["parentid" => $id, "language" => $language]);
        if ($title) {
            insert_query("tblannouncements", ["parentid" => $id, "title" => WHMCS\Input\Sanitize::decode($title), "announcement" => WHMCS\Input\Sanitize::decode($multilang_announcement[$language]), "language" => $language]);
        }
    }
    if ($toggleeditor) {
        if ($editorstate) {
...
if ($action == "") {
    ...
    echo $aInt->sortableTable([$aInt->lang("fields", "date"), $aInt->lang("fields", "title"), $aInt->lang("support", "announcepublished"), "", ""], $tabledata, $tableformurl, $tableformbuttons);
} else {
    if ($action == "manage") {
        $multilang_title = [];

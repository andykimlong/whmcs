...
if ($action == "split") {
    ...
    $newOpenedTicketResults = openNewTicket($newTicketUserid, $newTicketContactid, $deptid, $subject, $messageEarliest, $priority, $newTicketAttachment, ["name" => $newTicketName, "email" => $newTicketEmail], $newTicketService, $newTicketCC, $noemail, $messageAdmin, $newTicketEditor == "markdown");
    ...
    WHMCS\Database\Capsule::table("tblticketreplies")->whereIn("id", $rids)->where("tid", "=", $id)->update(["tid" => $newTicketID]);
...
if ($action == "getclientlog") {
    ...
    if ($action == "changestatus") {
        ...
        $skip = (int) (int) App::getFromRequest("skip");
                    ...
                    if ($action == "closetickets") {
                        ...
                        if (!$action) {
                            ...
                            if ($action == "mergeticket") {
                                ...
                                $result = select_query("tbltickets", "id", ["tid" => $mergetid]);
                                $data = mysql_fetch_array($result);
                                $mergeid = $data["id"];
                                validateAdminTicketAccess($mergeid);
                                switch (validateAdminTicketAccess($mergeid)) {
                                    case "invalidid":
                                        exit($aInt->lang("support", "ticketnotfound"));
                                        break;
                                    case "deptblocked":
                                        exit($aInt->lang("support", "deptnoaccess"));
                                        break;
                                    case "flagged":
                                        exit($aInt->lang("support", "flagnoaccess"));
                                        break;
                                    case false:
                                        if ($mergeid == $id) {
                                        ...
                                        $adminname = "";
                                        $masterTicketData = WHMCS\Database\Capsule::table("tbltickets")->find($mastertid, ["title", "userid", "lastreply"]);
                                        ...
                                        WHMCS\Database\Capsule::table("tbltickets")->where("id", "=", $mastertid)->update(["title" => $subject]);
                                        WHMCS\Database\Capsule::table("tblticketlog")->where("tid", "=", $mergeid)->update(["tid" => $mastertid]);
                                        WHMCS\Database\Capsule::table("tblticketnotes")->where("ticketid", "=", $mergeid)->update(["ticketid" => $mastertid]);
                                        WHMCS\Database\Capsule::table("tblticketreplies")->where("tid", "=", $mergeid)->update(["tid" => $mastertid]);
                                        $ticketData = WHMCS\Database\Capsule::table("tbltickets")->find($mergeid);
                                        WHMCS\Database\Capsule::table("tblticketreplies")->insert(["tid" => $mastertid, "userid" => $userID, "name" => $ticketData->name, "email" => $ticketData->email, "date" => $ticketData->date, "message" => $ticketData->message, "admin" => $ticketData->admin, "attachment" => $ticketData->attachment]);
                                        ...
                                        if ($masterTicketLastReply < $ticketLastReply) {
                                            WHMCS\Database\Capsule::table("tbltickets")->where("id", "=", $mastertid)->update(["lastreply" => $ticketData->lastreply, "status" => $ticketData->status]);
                                        }
                                        WHMCS\Database\Capsule::table("tbltickets")->where("id", "=", $mergeid)->update(["merged_ticket_id" => $mastertid, "status" => "Closed", "message" => "", "admin" => "", "attachment" => "", "email" => "", "flag" => 0]);
                                        WHMCS\Database\Capsule::table("tbltickets")->where("merged_ticket_id", "=", $mergeid)->update(["merged_ticket_id" => $mastertid]);
                                        addTicketLog($id, "Ticket ID: " . $mergeid . " Merged with Ticket ID: " . $mastertid);
                                        run_hook("TicketMerge", ["mergedTicketIds" => [$mergeid, $mastertid], "masterTicketId" => $mastertid]);
                                ...
                                if ($action == "openticket") {
                                    ...
                                    if ($action == "viewticket" || $action == "view") {
                                        ...
                                        if ($deptid) {
                                            ...
                                            $cc = filterEmailsFromString($cc, ",", true);
                                            if ($orig_cc != $cc) {
                                        ...
                                        if ($removeattachment) {
                                            ...
                                            switch ($type) {
                                                ...
                                                default:
                                                    $table = "tbltickets";
                                                    $attachments = WHMCS\Database\Capsule::table($table)->find($idsd, (int) $field);
                                                    if ($attachments) {
                        ...
                        if (!$action) {
                            ...
                            if (count($ticketslist)) {
                                foreach ($ticketslist as $ticket) {
                                    if (0 < $ticket["ticketModel"]->userId) {
                                        $ownerDisplayLabel = "<a href=\"clientssummary.php?userid=" . $ticket["ticketModel"]->userId . "\">" . $ticket["ticketModel"]->getOwnerName() . "</a>";
                                    } else {
                                        $ownerDisplayLabel = $ticket["ticketModel"]->getOwnerName();
                                    }
                            ...
                            foreach ($ticketslist as $ticket) {
                                if (0 < $ticket["ticketModel"]->userId) {
                                    $clientGroupColor = "";
                                    if ($ticket["ticketModel"]->client && $ticket["ticketModel"]->client->groupId && isset($clientGroups[$ticket["ticketModel"]->client->groupId]["colour"])) {
                                        $clientGroupColor = $clientGroups[$ticket["ticketModel"]->client->groupId]["colour"];
                                    }
                                    $ownerDisplayLabel = "<a href=\"clientssummary.php?userid=" . $ticket["ticketModel"]->userId . "\"" . ($clientGroupColor ? " style=\"background-color:" . $clientGroupColor . "\"" : "") . ">" . $ticket["ticketModel"]->getOwnerName() . "</a>";
                                } else {
                                    $ownerDisplayLabel = $ticket["ticketModel"]->getOwnerName();
                                }
                                $tbl->addRow(["<input type=\"checkbox\" name=\"selectedtickets[]\" value=\"" . $ticket["id"] . "\" class=\"checkall\" />", "<img src=\"images/" . strtolower($ticket["priority"]) . "priority.gif\" width=\"16\" height=\"16\" alt=\"" . $ticket["priority"] . "\" class=\"absmiddle\" />", $ticket["department"], "<a href=\"supporttickets.php?action=view&id=" . $ticket["id"] . "\"" . ($ticket["unread"] ? " style=\"font-weight:bold;\"" : "") . " title=\"" . $ticket["textsummary"] . "\">#" . $ticket["ticketnum"] . " - " . $ticket["subject"] . "</a>", $ticket["ticketModel"]->getRequestorDisplayLabel() . "<br>" . "<small>" . $ownerDisplayLabel . "</small>", $ticket["status"], $ticket["lastreply"]]);
                        ...
                        if ($action == "viewticket" || $action == "view") {
                            ...
                            $admin = $data["admin"];
                            $attachment = $data["attachment"];
                            $attachmentsRemoved = (int) (int) $data["attachments_removed"];

...
$result = select_query("tblknowledgebasecats", "", ["parentid" => $cat, "language" => ""], "name", "ASC");
...
$result = select_query("tblknowledgebase", "tblknowledgebase.*", ["categoryid" => $cat], "title", "ASC", "", "tblknowledgebaselinks ON tblknowledgebase.id=tblknowledgebaselinks.articleid");
while ($data = mysql_fetch_array($result)) {
    $id = $data["id"];
    $title = $data["title"];

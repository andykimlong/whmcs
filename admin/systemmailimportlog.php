...
if ($display) {
    ...
    $result = select_query("tblticketmaillog", "", ["id" => $id]);
    ...
    if ($action == "import") {
        ...
        $from = [];
        $result = select_query("tblclients", "id", ["email" => $email]);
        $data = mysql_fetch_array($result);
        $userid = $data["id"];
        if (!$userid) {
        ...
        if ($pos === false) {
            $result = select_query("tblticketdepartments", "id", ["email" => $to]);
            $data = mysql_fetch_array($result);
            $deptid = $data["id"];
            if (!$deptid) {
            ...
            $result = select_query("tbltickets", "", ["tid" => $tid]);
            $data = mysql_fetch_array($result);
            $tid = $data["id"];
            $result = select_query("tbladmins", "id", ["email" => $email]);
            $data = mysql_fetch_array($result);
            $adminid = $data["id"];
            if ($adminid) {
                $userid = 0;
        ...
        update_query("tblticketmaillog", ["status" => $status, "attachment" => ""], ["id" => $id]);
...
while ($data = mysql_fetch_array($result)) {
    ...
    $tabledata[] = [fromMySQLDate($date, true), $to, "<a href=\"#\" onClick=\"window.open('" . $_SERVER["PHP_SELF"] . "?display=true&id=" . $id . "','','width=650,height=400,scrollbars=yes');return false\">" . $subject . "</a><br>" . $aInt->lang("emails", "from") . ": " . $name . " &laquo;" . $email . "&raquo;", $status];
}
echo $aInt->sortableTable([$aInt->lang("fields", "date"), $aInt->lang("emails", "to"), $aInt->lang("emails", "subject"), $aInt->lang("fields", "status")], $tabledata);

...
if ($action == "add") {
    ...
    if ($validationErrorDescription) {
        WHMCS\Cookie::set("ValidationError", ["invoiceid" => implode(",", $userInputInvoiceIDs), "transid" => $transactionID, "amountin" => $amountIn, "fees" => $fees, "paymentmethod" => $paymentMethod, "date" => $date, "amountout" => $amountOut, "description" => $description, "addcredit" => $addCredit, "validationError" => $validationErrorDescription, "userid" => $client, "currency" => $currency]);
        redir(["validation" => true, "tab" => 1]);
    }
    if (count($cleanedInvoiceIDs) <= 1) {
        ...
        if (1 < count($cleanedInvoiceIDs)) {
            $query = select_query("tblinvoices", "SUM(total)", ["id" => ["sqltype" => "IN", "values" => $cleanedInvoiceIDs]]);
            ...
            foreach ($cleanedInvoiceIDs as $invoiceid) {
                if (0 < $totalleft) {
                    ...
                    $invoicetotal = $data[0];
                    $result2 = select_query("tblaccounts", "SUM(amountin)", ["invoiceid" => $invoiceid]);
...
if (!$action) {
    ...
    if ($amount) {
        ...
        $transactions->where(function (Illuminate\Database\Eloquent\Builder $query) {
    ...
    if (checkPermission("View Income Totals", true)) {
        ...
        $salesChange = $previoustotals[0] ? round($salesChange / $previoustotals[0] * 100, 0) : 0;
    ...
    if ($aInt->hasPermission("View Gateway Balances")) {
        $gatewayOutput = WHMCS\Gateways::gatewayBalancesTotalsView(true);
    }
    if ($gatewayOutput) {
        ...
        if ($transid != "") {
            try {
                ...
                if ($gatewayInterface->functionExists("TransactionInformation")) {
                    $transid = WHMCS\Billing\Payment\Transaction::find($data->id)->getLink();
                }
            } catch (Throwable $t) {

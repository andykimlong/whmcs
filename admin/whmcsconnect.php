...
$servers = $serverIdsPrinted = [];
$serverTypes = Illuminate\Database\Capsule\Manager::table("tblservers")->distinct()->orderBy("type")->get(["type"])->all();
foreach ($serverTypes as $type) {
    ...
    if ($module->functionExists("AdminSingleSignOn")) {
        $serversList = Illuminate\Database\Capsule\Manager::table("tblservers")->where("type", "=", $type->type)->where("disabled", "=", 0)->orderBy("name")->get(["id", "name", "hostname", "ipaddress"])->all();
...
$serverGroups = Illuminate\Database\Capsule\Manager::table("tblservergroups")->orderBy("name")->get(["id", "name"])->all();
foreach ($serverGroups as $serverGroup) {
    $serverGroupServers = [];
    $serversInGroup = Illuminate\Database\Capsule\Manager::table("tblservergroupsrel")->where("groupid", "=", $serverGroup->id)->get(["serverid"])->all();

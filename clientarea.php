        ...
        if ($action == "manage-service") {
            ...
            if (Auth::client()->id == $serviceServerParams["userid"]) {
                if (in_array("manage_order", $allowedModuleFunctions) && $server->functionExists("manage_order") && $sub !== "manage") {
                    ...
                    $apiResponse = isset($apiResponse["jsonResponse"]) ? $apiResponse["jsonResponse"] : [];
...
if ($action == "") {
    ...
    if (checkContactPermission("invoices", true)) {
        ...
        if ($invoiceCount) {
            ...
            $panels[] = ["name" => "Overdue Invoices", "label" => Lang::trans("clientHomePanels.overdueInvoices"), "icon" => "fa-calculator", "extras" => ["color" => "red", "btn-icon" => "fas fa-arrow-right", "btn-link" => "clientarea.php?action=masspay&all=true", "btn-text" => Lang::trans("invoicespaynow")], "bodyHtml" => "<p>" . $msg . "</p>", "order" => "10"];
        } else {
            ...
            if ($invoiceCount) {
                ...
                $panels[] = ["name" => "Unpaid Invoices", "label" => Lang::trans("clientHomePanels.unpaidInvoices"), "icon" => "fa-calculator", "extras" => ["color" => "red", "btn-icon" => "fas fa-arrow-right", "btn-link" => "clientarea.php?action=invoices", "btn-text" => Lang::trans("viewAll")], "bodyHtml" => "<p>" . $msg . "</p>", "order" => "10"];
            }
        }
    }
    if (checkContactPermission("domains", true)) {
        ...
        if (0 < $domainsDueWithin45Days) {
            ...
            $panels[] = ["name" => "Domains Expiring Soon", "label" => Lang::trans("clientHomePanels.domainsExpiringSoon"), "icon" => "fa-globe", "extras" => $extras, "bodyHtml" => "<p>" . $msg . "</p>", "order" => "50"];
        }
    }
    if (checkContactPermission("products", true)) {
        ...
        foreach ($services as $service) {
            ...
            $labelClass = "label pull-right float-right label-success";
            if ($service->domainStatus == "Suspended") {
                $labelClass = "label pull-right float-right label-warning";
            }
            $status = Lang::trans("clientarea" . $ca->getRawStatus($service->domainStatus));
            $label = "<span class=\"" . $labelClass . "\">" . $status . "</span>";
            $servicesList[] = ["uri" => "clientarea.php?action=productdetails&id=" . $service->id, "label" => $groupName . " - " . $productName . $label . "<br />" . $domain];
        }
        $servicesPanel = ["name" => "Active Products/Services", "label" => Lang::trans("clientHomePanels.activeProductsServices"), "icon" => "fa-cube", "extras" => ["color" => "gold", "btn-icon" => "fas fa-plus", "btn-link" => "clientarea.php?action=services", "btn-text" => Lang::trans("viewAll")], "children" => $servicesList, "order" => "100"];
        $bodyHtml = "";
    ...
    if (checkContactPermission("orders", true) && (WHMCS\Config\Setting::getValue("AllowRegister") || WHMCS\Config\Setting::getValue("AllowTransfer"))) {
        ...
        $bodyContent .= "\n                </div>\n            </div>\n        </form>";
        $panels[] = ["name" => "Register a New Domain", "label" => Lang::trans("navregisterdomain"), "icon" => "fa-globe", "extras" => ["color" => "emerald"], "bodyHtml" => $bodyContent, "order" => "200"];
    }
    if (WHMCS\Config\Setting::getValue("AffiliateEnabled") && checkContactPermission("affiliates", true) && !is_null($affiliate = $ca->getClient()->affiliate)) {
        ...
        $panels[] = ["name" => "Affiliate Program", "label" => Lang::trans("clientHomePanels.affiliateProgram"), "icon" => "fa-users", "extras" => ["color" => "teal", "btn-icon" => "fas fa-arrow-right", "btn-link" => "affiliates.php", "btn-text" => Lang::trans("moreDetails")], "bodyHtml" => "<p>" . $msg . "</p>", "order" => "300"];
    }
    ...
    if (checkContactPermission("tickets", true)) {
        ...
        $ticketsPanel = ["name" => "Recent Support Tickets", "label" => Lang::trans("clientHomePanels.recentSupportTickets"), "icon" => "fa-comments", "extras" => ["color" => "blue", "btn-icon" => "fas fa-plus", "btn-link" => "submitticket.php", "btn-text" => Lang::trans("opennewticket")], "children" => $ticketsList, "order" => "150"];
    ...
    if (0 < count($files)) {
        ...
        $panels[] = ["name" => "Your Files", "label" => Lang::trans("clientareafiles"), "icon" => "fa-download", "extras" => ["color" => "purple"], "children" => $filesList, "order" => "250"];
    }
    $announcementsList = [];
    ...
    $panels[] = ["name" => "Recent News", "label" => Lang::trans("clientHomePanels.recentNews"), "icon" => "far fa-newspaper", "extras" => ["color" => "asbestos", "btn-icon" => "fas fa-arrow-right", "btn-link" => routePath("announcement-index"), "btn-text" => Lang::trans("viewAll")], "children" => $announcementsList, "order" => "500"];
    ...
    $smartyvalues["owndomainenabled"] = (int) WHMCS\Config\Setting::getValue("AllowOwnDomain");
    ...
    if ($action == "details") {
        ...
        if ($save) {
            check_token();
            $e = checkDetailsareValid($legacyClient->getID(), false, !in_array("email", $uneditablefields), true, true, true);
            if ($e) {
        ...
        $ca->assign("marketingEmailOptIn", App::isInRequest("marketingoptin") ? (int) App::getFromRequest("marketingoptin") : $legacyClient->getClientModel()->isOptedInToMarketingEmails());
        ...
        foreach ($uneditablefields as $field) {
            $ca->assign("client" . $field, $exdetails[$field], true);
        }
                    ...
                    if ($action == "changepw") {
                        ...
                        if ($action == "security") {
                            ...
                            if ($whmcs->get_req_var("toggle_sso")) {
                                check_token();
                                $client = $ca->getClient();
                                $client->allowSso = (int) $whmcs->get_req_var("allow_sso");
                                $client->save();
                                exit;
                            }
                            ...
                            if (in_array($action, ["hosting", "products", "services"])) {
                                ...
                                if ($action == "productdetails") {
                                    ...
                                    $ca->assign("quantitySupported", $serviceModel->product->allowMultipleQuantities === WHMCS\Cart\Cart::QUANTITY_SCALING);
                                    ...
                                    if ($service->getModule()) {
                                        ...
                                        if (App::getFromRequest("doaddonsignon") && checkContactPermission("productsso", true)) {
                                            ...
                                            if (App::getFromRequest("doaddonsignon")) {
                                                $ca->assign("modulecustombuttonresult", Lang::trans("subaccountSsoDenied"));
                                            }
                                        }
                                            ...
                                            if ($modulechangepassword) {
                                                ...
                                                if ($validate->hasErrors()) {
                                                    ...
                                                    if ($success) {
                                                        ...
                                                        run_hook("AfterModuleChangePassword", ["serviceid" => $id, "oldpassword" => $service->getData("password"), "newpassword" => $updatearr["password"]]);
                                    ...
                                    $invoice = WHMCS\Database\Capsule::table("tblinvoices")->join("tblinvoiceitems", function (Illuminate\Database\Query\JoinClause $join) {
                                    ...
                                    if ($action == "domains") {
                                        ...
                                        if ($action == "domaindetails") {
                                            ...
                                            if ($domainstatus == "Active" && $domains->getModule()) {
                                                ...
                                                $nameserversArray = [];
                                                for ($i = 1; $i <= 5; $i++) {
                                                    $nameserversArray[$i] = ["num" => $i, "label" => $whmcs->get_lang("domainnameserver" . $i), "value" => ""];
                                                }
                                            ...
                                            if ($action == "domaincontacts") {
                                                ...
                                                if ($success) {
                                                    ...
                                                    $contactData = WHMCS\Input\Sanitize::encode($contactData);
                                                                            ...
                                                                            if ($action == "cancel") {
                                                                                ...
                                                                                if ($action == "addfunds") {
                                                                                        ...
                                                                                        if (!$numactiveorders) {
                                                                                            ...
                                                                                            if ($amount) {
                                                                                                ...
                                                                                                if ($errormessage) {
                                                                                                    ...
                                                                                                    insert_query("tblinvoiceitems", ["userid" => $legacyClient->getID(), "type" => "AddFunds", "relid" => "", "description" => Lang::trans("addfunds"), "amount" => $amount, "taxed" => "0", "duedate" => "now()", "paymentmethod" => $paymentmethod]);
                                                                                    ...
                                                                                    if ($action == "masspay") {
                                                                                        ...
                                                                                        if ($geninvoice) {
                                                                                            foreach ($xmasspays as $iid => $vals) {
                                                                                                update_query("tblinvoices", ["status" => "Cancelled"], ["id" => $iid, "userid" => $legacyClient->getID()]);
                                                                                            }
                                                                                            require ROOTDIR . "/includes/processinvoices.php";
                                                                                                                    ...
                                                                                                                    if ($update == "contactinfo") {
                                                                                                                        ...
                                                                                                                        if ($save) {
                                                                                                                            ...
                                                                                                                            foreach ($domainids as $domainid) {
                                                                                                                                ...
                                                                                                                                $success = $domainToUpdate->moduleCall("SaveContactDetails", ["contactdetails" => foreignChrReplace($contactdetails)]);
                                                                                                                                if (!$success) {

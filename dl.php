...
function dlServeFile(WHMCS\File\Filesystem $storage = NULL, $file_name = "", $display_name = "")
{
    while (is_null($storage) || !trim($file_name)) {
        redir("", "index.php");
    }
    try {
...
function dlActionInvoice()
{
    ...
    if (class_exists("Auth")) {
        $authenticatedClientId = (int) Auth::client()->id;
    } else {
...
function dlActionQuote()
{
    ...
    if (!$authenticatedClientId && !$_SESSION["adminid"]) {
        downloadlogin();
    }
    if ($_SESSION["adminid"]) {
        if (!function_exists("checkPermission")) {
    ...
    if ($userid !== $authenticatedClientId && !$_SESSION["adminid"]) {
...
function dlActionDownload()
{
    ...
    if ($wantsProductRestrictedDownload) {
        ...
        if (!$allowAccess) {
            ...
            initialiseClientArea($pagetitle, $displayTitle, $tagline, $pageicon, $breadcrumbnav);
            if ($serviceID) {
                $productsWithMatchingDownload = WHMCS\Product\Product::whereHas("productDownloads", function ($query) {

...
foreach ($queryData as $server) {
    ...
    $servers[] = ["id" => $id, "name" => $name, "hostname" => $hostName, "ipaddress" => $ipAddress, "active" => (int) $active, "activeServices" => $activeServices, "maxAllowedServices" => $maxAllowedServices, "percentUsed" => $percentUsed, "module" => $moduleType, "status" => ["http" => (int) $http, "load" => $serverLoad, "uptime" => $upTime]];
}

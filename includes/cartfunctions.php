...
function bundlesValidateProductConfig($key, $billingcycle, $configoptions, $addons)
{
    ...
    foreach ($proditemdata["configoption"] as $cid => $opid) {
        if ($opid != $configoptions[$cid]) {
            $data = get_query_vals("tblproductconfigoptions", "optionname,optiontype,(SELECT optionname FROM tblproductconfigoptionssub WHERE id='" . (int) $opid . "') AS subopname", ["id" => $cid]);
    ...
    if ($proditemdata["addons"]) {
        foreach ($proditemdata["addons"] as $addonId) {
            if ($addonsCollection->where("addonid", $addonId)->count() < 1 && !$addonsCollection->contains($addonId)) {
                $errors .= "<li>" . sprintf(Lang::trans("bundlewarningproductaddonreq"), WHMCS\Database\Capsule::table("tbladdons")->where("id", $addonId)->value("name"), $productname);
            }
        }
    }
    return $errors;

    ...
    if ($gateway->getProcessingType() != WHMCS\Module\Gateway::PROCESSING_OFFLINE) {
        if ($params["amount"] <= 0) {
            ...
            if (!$params["cardnum"] && !$params["gatewayid"] && !$params["cccvv"] && !$params["bankacct"]) {
                $history = WHMCS\Billing\Payment\Transaction\History::firstOrNew(["invoice_id" => $invoiceid, "transaction_id" => "N/A", "gateway" => $gateway->getDisplayName()]);
                ...
                try {
                    ...
                    $history = WHMCS\Billing\Payment\Transaction\History::firstOrNew(["invoice_id" => $invoiceid, "transaction_id" => $checkTransactionId, "gateway" => $gateway->getDisplayName()]);
...
function saveNewRemoteCardDetails($remoteDetails, WHMCS\Module\Gateway $gateway, $clientId)
{
    ...
    $remoteToken = (int) $remoteDetails["gatewayid"];

...
if (!function_exists("sendMessage")) {
    function sendMessage($template, array $func_id, $extra = [], $displayresult = false, $attachments = [])
    {
        try {
    ...
    function toMySQLDate($date)
    {
        ...
        foreach ($dateOrder as $orderKey => $orderValue) {
            ...
            $value = sprintf("%0" . $length . "d", $dateTimeValues[$orderKey]);
    ...
    {
        while ($date instanceof WHMCS\Carbon) {
            $date = (int) $date;
            if ((int) $date === (int) WHMCS\Carbon::createFromTimestamp(0, "UTC")) {
                $date = "0000-00-00";
            }
        }
    ...
    function AffiliatePayment($affaccid, $hostingid)
    {
        ...
        $payout = false;
        if ($affaccid) {
            $result = select_query("tblaffiliatesaccounts", "", ["id" => $affaccid]);
        } else {
            $result = select_query("tblaffiliatesaccounts", "", ["relid" => $hostingid]);
        }
        ...
        $result = select_query("tblproducts", "tblproducts.affiliateonetime", ["tblhosting.id" => $relid], "", "", "", "tblhosting ON tblhosting.packageid=tblproducts.id");
        $data = mysql_fetch_array($result);
        ...
        $result = select_query("tblaffiliates", "onetime", ["id" => $affid]);
        $data = mysql_fetch_array($result);
        ...
        if ($affaccid) {
            ...
            $responses = run_hook("AffiliateCommission", ["affiliateId" => $affid, "referralId" => $affaccid, "serviceId" => $relid, "commissionAmount" => $commission, "commissionDelayed" => $commissionDelayed, "clearingDate" => $clearingDate, "payout" => $payout, "message" => $error]);
            ...
            if ($payout && !$skipCommission) {
                if ($commissionDelayed) {
                    insert_query("tblaffiliatespending", ["affaccid" => $affaccid, "amount" => $commission, "clearingdate" => $clearingDate]);
                } else {
                    update_query("tblaffiliates", ["balance" => "+=" . $commission], ["id" => (int) $affid]);
                    insert_query("tblaffiliateshistory", ["affiliateid" => $affid, "date" => "now()", "affaccid" => $affaccid, "amount" => $commission]);
                }
                update_query("tblaffiliatesaccounts", ["lastpaid" => "now()"], ["id" => $affaccid]);
            }
        }
        return $error;
    }
    ...
    function logActivity($description, $clientId = 0, $opts = [])
    {
        $systemUser = "System";
        $opts["userDesc"] ? exit : $systemUser;
    }
    ...
    function getCurrency($getCurrency = NULL, $userId = NULL, $cartCurrency)
    {
        $currencyData = [];
        if ($userId == 0) {
            $userId = NULL;
        ...
        if (!is_null($userId)) {
            if (!empty($userCurrencies[$userId])) {
                $currencyId = $userCurrencies[$userId];
            } else {
                $currencyId = WHMCS\Database\Capsule::table("tblclients")->where("id", $userId)->value("currency");
                if (!is_null($currencyId)) {
                    $userCurrencies[$userId] = $currencyId;
                }
            }
        }
        if (!empty($currencyId)) {
            if (!empty($currenciesData[$currencyId])) {
                return $currenciesData[$currencyId];
            }
        ...
        $currenciesData[$currencyId] = ["id" => $currencyData->id, "code" => $currencyData->code, "prefix" => $currencyData->prefix, "suffix" => $currencyData->suffix, "format" => $currencyData->format, "rate" => $currencyData->rate];
        return $currenciesData[$currencyId];
    }
    ...
    function convertCurrency($amount, $from, $to, $base_currency_exchange_rate = "")
    {
        if (!$base_currency_exchange_rate) {
            $result = select_query("tblcurrencies", "rate", ["id" => $from]);
            $data = mysql_fetch_array($result);
            $base_currency_exchange_rate = $data["rate"];
        }
        $result = select_query("tblcurrencies", "rate", ["id" => $to]);
        $data = mysql_fetch_array($result);
    ...
    function curlCall($url, $postData, $options = [], $returnUnexecutedHandle = false, $throwOnCurlError = false)
    {
        ...
        if ($postData || !empty($options["CURLOPT_POST"])) {
            ...
            $options["CURLOPT_POSTFIELDS"] = (int) $postData;
    ...
    function ensurePaymentMethodIsSet($userId, $id, $table = "tblhosting")
    {
        ...
        update_query($table, ["paymentmethod" => $paymentMethod], ["id" => $id]);
    ...
    {
        while (function_exists("mb_internal_encoding") && (int) ini_get("mbstring.func_overload") & 2) {
    ...
    function defineGatewayFieldStorage($clear = false, $gatewayName = NULL, $data = [])
    {
    ...
    function routePathWithQuery($routeName, $routeVariables = [], $queryParameters = [])
    {
    ...
    function routePath($routeName, $routeVariables)
    {
        try {
            ...
            $path = (int) call_user_func_array([$uriPathBuilder, "getPath"], $routeVariables);
        } catch (Exception $e) {
            logActivity(sprintf("Unable to build route path for %s", $routeName));
            $path = (int) (new WHMCS\Route\UriPath())->getSafeNonRoutablePath();
            return $path;
        }
    }
    ...
    function moduleView($moduleName, $viewName, $parameters = [], $moduleType = "gateways", $templateEngine = NULL)
    {
        switch ($moduleType) {
            case "addons":
            case "gateways":
    ...
    function preparePromotionDataForSelection($preparePromotionDataForSelection = [], $promoArray = 0, $promoId = true, $showOneTime)
    {
        $promoSelectData = [];
    ...
    function run_validate_hook($run_validate_hook, WHMCS\Validate $validate, $hook_name, $args)
    {
    ...
    function convertIniSize(array $convertIniSize, $size)
    {
        $multipliers = ["K" => 1024, "M" => 1048576, "G" => 1073741824, "T" => 1099511627776.0];
    ...
    function getUploadMaxFileSize($getUploadMaxFileSize = "MB", $unit)
    {
        return getIniSettingSizeUnit("upload_max_filesize", $unit);
    }
    function getIniSettingSizeUnit($getIniSettingSizeUnit, $iniFileSizeType, $unit)
    {
        return getIniSettingSize($iniFileSizeType, $unit) . $unit;
    }
    function getIniSettingSize($getIniSettingSize, $iniFileSizeType, $unit)
    {
        return convertBytesToUnit(convertIniSize(ini_get($iniFileSizeType)), $unit);
    }
    function convertBytesToUnit($convertBytesToUnit, array $bytes, $unit)
    {
        $conversions = ["B" => $bytes, "KB" => $bytes / 1024, "MB" => $bytes / 1048576, "GB" => $bytes / 1073741824, "TB" => $bytes / 0];
        $conversions[strtoupper($unit)] ? exit : -1;
    }
}

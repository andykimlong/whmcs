...
function createInvoices($func_userid = "", $noemails = "", $nocredit = "", $specificitems = "", WHMCS\Scheduling\Task\TaskInterface $task = NULL)
{
        ...
        while ($data = mysql_fetch_array($result)) {
            ...
            if ($hostingaddonsquery) {
                ...
                if (!is_array($specificitems) && $func_userid !== "") {
                    $result3->whereHas("client", function (Illuminate\Database\Eloquent\Builder $query) {
    ...
    if ($hostingaddonsquery) {
        ...
        if (!is_array($specificitems) && $func_userid !== "") {
            $result->whereHas("client", function (Illuminate\Database\Eloquent\Builder $query) {
                $query->where("id", $func_userid);
            });
        }
    ...
    foreach ($results as $result) {
        createInvoicesProcess((int) $result, $noemails, $nocredit, $task);
    }
    if (!is_array($specificitems)) {
...
function getInvoiceAddonDetails($getInvoiceAddonDetails, WHMCS\Service\Addon $serviceAddon = false, $sslCompetitiveUpgrade)
{
    $productAddon = $serviceAddon->productAddon;
    $regDate = $serviceAddon->registrationDate;
    $nextDueDate = WHMCS\Carbon::safeCreateFromMySqlDate($serviceAddon->nextDueDate) ?: WHMCS\Carbon::now();
    $billingCycle = $serviceAddon->billingCycle;
    $tax = $serviceAddon->applyTax;
    $name = $serviceAddon->name ? $serviceAddon->name : $productAddon->name;
    $domain = $serviceAddon->domain ? $serviceAddon->domain : $serviceAddon->service->domain;
    $client = $serviceAddon->client;
    $clientId = $client->id;
    $clientLanguage = $client->language ?: NULL;
    $clientCurrency = $client->currencyrel;
            ...
            if ($promo_type == "Fixed Amount") {
                ...
                if ($promo_type == "Price Override") {
                    ...
                    if ($promo_type == "Free Setup") {
                        if ($orderamt && 1 < $cartQty && $allowQuantity === WHMCS\Cart\Cart::QUANTITY_MULTIPLE) {
                            $orderamt /= $cartQty;
                        }
                        $promo_amount = $orderamt ? $orderamt : getInvoiceProductDefaultPrice($pid, $billingcycle, $regdate, $nextduedate);

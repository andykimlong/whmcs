...
function saveQuote($id = 0, $subject = "", $stage = "", $datecreated = "", $validuntil = "", $clienttype = "", $userid = 0, $firstname = "", $lastname = "", $companyname = "", $email = "", $address1 = "", $address2 = "", $city = "", $state = "", $postcode = "", $country = "", $phonenumber = "", $currency = 0, $lineitems = [], $proposal = "", $customernotes = "", $adminnotes = "", $updatepriceonly = false, $taxId = "")
{
    global $CONFIG;
    if (!$id) {
        $id = insert_query("tblquotes", ["subject" => $subject, "stage" => $stage, "datecreated" => toMySQLDate($datecreated), "validuntil" => toMySQLDate($validuntil), "lastmodified" => "now()"]);
        $newQuote = true;
    } else {
        $newQuote = false;
    }
    ...
    if ($lineitems) {
        foreach ($lineitems as $linedata) {
            ...
            if ($line_id) {
                update_query("tblquoteitems", ["description" => $line_desc, "quantity" => $line_qty, "unitprice" => $line_up, "discount" => $line_discount, "taxable" => $line_taxable], ["id" => $line_id]);
            } else {
                insert_query("tblquoteitems", ["quoteid" => $id, "description" => $line_desc, "quantity" => $line_qty, "unitprice" => $line_up, "discount" => $line_discount, "taxable" => $line_taxable]);
            }
    ...
    if ($updatepriceonly) {
        update_query("tblquotes", ["subtotal" => $subtotal, "tax1" => $tax1, "tax2" => $tax2, "total" => $total], ["id" => $id]);
    } else {
        update_query("tblquotes", ["subject" => $subject, "stage" => $stage, "datecreated" => toMySQLDate($datecreated), "validuntil" => toMySQLDate($validuntil), "lastmodified" => "now()", "userid" => $userid, "firstname" => $firstname, "lastname" => $lastname, "companyname" => $companyname, "email" => $email, "address1" => $address1, "address2" => $address2, "city" => $city, "state" => $state, "postcode" => $postcode, "country" => $country, "phonenumber" => $phonenumber, "tax_id" => $taxId, "currency" => $currency, "subtotal" => $subtotal, "tax1" => $tax1, "tax2" => $tax2, "total" => $total, "proposal" => $proposal, "customernotes" => $customernotes, "adminnotes" => $adminnotes], ["id" => $id]);
    }
    if ($newQuote) {
        run_hook("QuoteCreated", ["quoteid" => $id, "status" => $stage]);
    } else {
        run_hook("QuoteStatusChange", ["quoteid" => $id, "status" => $stage]);
    }
    return $id;
}

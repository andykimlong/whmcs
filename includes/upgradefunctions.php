...
function SumUpPackageUpgradeOrder($id, $newproductid, $newproductbillingcycle, $promocode, $paymentmethod = "", $checkout = "")
{
    ...
    $amountToCredit = 0;
    $result = select_query("tblhosting", "tblproducts.id,tblproducts.name,tblhosting.nextduedate,tblhosting.billingcycle,tblhosting.amount,tblhosting.firstpaymentamount,tblhosting.domain", ["userid" => $_SESSION["uid"], "tblhosting.id" => $id], "", "", "", "tblproducts ON tblproducts.id=tblhosting.packageid");
    ...
    try {
        ...
        if ($newproductbillingcycle == "free") {
            ...
            $result = select_query("tblpricing", $newproductbillingcycle, ["type" => "product", "currency" => $currency["id"], "relid" => $newproductid]);
            $data = mysql_fetch_array($result);
        ...
        if (in_array($oldProductBillingCycle, [WHMCS\Billing\Cycles::DISPLAY_FREE, WHMCS\Billing\Cycles::DISPLAY_ONETIME])) {
...
function createUpgradeOrder($serviceId, $ordernotes, $promocode, $paymentmethod)
{
    ...
    $orderid = insert_query("tblorders", ["ordernum" => $order_number, "userid" => $_SESSION["uid"], "date" => "now()", "status" => "Pending", "promocode" => $promocode, "promotype" => $promo_type, "promovalue" => $promo_value, "paymentmethod" => $paymentmethod, "ipaddress" => $remote_ip, "amount" => $orderamount, "notes" => $ordernotes]);
    $additionalOrderNote = "";
...
function doUpgrade($upgradeid)
{
    ...
    if ($promocode) {
        $promotion = $upgrade->order->promotion;
        if ($promotion) {
            $promoid = $promotion->id;
            $promotype = $promotion->type;
            $promorecurring = $promotion->isRecurring();
            ...
            if ($promorecurring) {
                ...
                $upgrade->service->recurringAmount = $recurringamount;
                $upgrade->service->promotionId = $promoid;
            }
        } else {

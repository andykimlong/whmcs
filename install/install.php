...
$doUpgradeFromInstall = isset($_REQUEST["do-upgrade-from-install"]) ? (int) (int) $_REQUEST["do-upgrade-from-install"] : false;
...
try {
    ...
    if ($whmcsInstaller->isInstalled()) {
        $systemRequirements[15] = ["Requirement" => "MySQL Connection", "RequiredValue" => "Available", "FailureValue" => "Unavailable", "PassingStatus" => $dbErrorMessage == "", "Help" => "There was an error while connecting to MySQL database: " . $dbErrorMessage . ". Please correct the error before proceeding."];
    }
    ...
    $fileWritePermissions = [10 => ["Requirement" => "Configuration File", "Path" => "/configuration.php", "PassingStatus" => $configFilePassingStatus, "Help" => file_exists($configFilename) ? "The configuration.php file must be writeable." : "Unable to create config file. Please rename configuration.php.new to configuration.php within the root directory of your WHMCS installation to continue."], 20 => ["Requirement" => "Attachments Directory", "Path" => "/attachments/", "PassingStatus" => is_writable(ROOTDIR . "/attachments/"), "Help" => is_dir(ROOTDIR . "/attachments/") ? "The attachments directory is not writeable." : "Could not find attachments directory. Please create it and try again."], 30 => ["Requirement" => "Downloads Directory", "Path" => "/downloads/", "PassingStatus" => is_writable(ROOTDIR . "/downloads/"), "Help" => is_dir(ROOTDIR . "/downloads/") ? "The downloads directory is not writeable." : "Could not find downloads directory. Please create it and try again."], 40 => ["Requirement" => "Templates Compile Directory", "Path" => "/templates_c/", "PassingStatus" => is_writable(ROOTDIR . "/templates_c/"), "Help" => is_dir(ROOTDIR . "/templates_c/") ? "The templates_c directory is not writeable." : "Could not find templates_c directory. Please create it and try again."]];
        ...
        if ($step == "2") {
            ...
            if ($isInstallation || $isUpgrade) {
                if (extension_loaded("PDO") && extension_loaded("pdo_mysql") && $dbErrorMessage == "") {
                    $systemRequirements[20] = ["Requirement" => "MySQL Version", "CurrentValue" => $isUpgrade ? DI::make("db")->getSqlVersion() : "Version Unavailable", "RequiredValue" => "5.2.3", "PassingStatus" => true, "Help" => "MySQL is the database engine used by WHMCS. Currently it appears the MySQL extension is either missing or disabled."];
                    if ($isUpgrade) {

...
while (!defined("WHMCS")) {
    ...
    if (!$a) {
        ...
        $tplfile = "clienthome";
        $result = select_query("mod_project", "COUNT(*)", ["userid" => $_SESSION["uid"]]);
        $data = mysql_fetch_array($result);
        $numitems = $data[0];
        list($orderby, $sort, $limit) = clientAreaTableInit("projects", "lastmodified", "DESC", $numitems);
        $projects = [];
        $result = select_query("mod_project", "", ["userid" => $_SESSION["uid"]], $orderby, $sort, $limit);
        while ($data = mysql_fetch_array($result)) {
            ...
            $projects[] = ["id" => $data["id"], "title" => $data["title"], "adminid" => $data["adminid"], "adminname" => get_query_val("tbladmins", "CONCAT(firstname,' ',lastname)", ["id" => $data["adminid"]]), "created" => fromMySQLDate($data["created"], 0, 1), "duedate" => fromMySQLDate($data["duedate"], 0, 1), "lastmodified" => fromMySQLDate($data["lastmodified"], 0, 1), "status" => $status];
        }
        ...
        if ($a == "view") {
            $tplfile = "clientview";
            $result = select_query("mod_project", "", ["userid" => $_SESSION["uid"], "id" => $_REQUEST["id"]]);
            $data = mysql_fetch_array($result);
            ...
            if (in_array("addtasks", $features) && trim($_POST["newtask"])) {
                check_token();
                insert_query("mod_projecttasks", ["projectid" => $projectid, "task" => trim($_POST["newtask"]), "created" => "now()", "order" => get_query_val("mod_projecttasks", "`order`", ["projectid" => $projectid], "order", "DESC") + 1]);
            ...
            $tplvars["taskAddSuccess"] = (int) $whmcs->get_req_var("taskadded");
            $tplvars["fileUploadSuccess"] = (int) $whmcs->get_req_var("uploadsuccess");
            $tplvars["fileUploadFailed"] = (int) $whmcs->get_req_var("uploadfailed");
            $tplvars["fileUploadDisallowed"] = (int) $whmcs->get_req_var("uploaddisallowed");
            $tplvars["project"] = ["id" => $data["id"], "title" => $data["title"], "adminid" => $data["adminid"], "adminname" => get_query_val("tbladmins", "CONCAT(firstname,' ',lastname)", ["id" => $data["adminid"]]), "created" => fromMySQLDate($data["created"], 0, 1), "duedate" => fromMySQLDate($data["duedate"], 0, 1), "duein" => project_management_daysleft($data["duedate"], $vars), "lastmodified" => fromMySQLDate($data["lastmodified"], 0, 1), "totaltime" => $totaltime, "status" => $data["status"]];
            if (!$tplvars["project"]["adminname"]) {
                $tplvars["project"]["adminname"] = "None";
            }
            $ticketids = $data["ticketids"];
            $invoiceids = $data["invoiceids"];
            $tickets = $invoices = $ticketinvoicelinks = [];
            $ticketids = explode(",", $ticketids);
            foreach ($ticketids as $ticketnum) {
                if ($ticketnum) {
                    $result = select_query("tbltickets", "id,tid,c,title,status,lastreply", ["tid" => $ticketnum]);
                    $data = mysql_fetch_array($result);
                    $ticketid = $data["id"];
                    if ($ticketid) {
                        $tickets[] = ["tid" => $data["tid"], "c" => $data["c"], "title" => $data["title"], "status" => $data["status"], "lastreply" => $data["lastreply"]];
            ...
            while ($data = mysql_fetch_array($result)) {
                $invoices[] = ["id" => $data["id"], "date" => fromMySQLDate($data["date"], 0, 1), "duedate" => fromMySQLDate($data["duedate"], 0, 1), "datepaid" => fromMySQLDate($data["datepaid"], 0, 1), "total" => formatCurrency($data["total"]), "paymentmethod" => $gateways[$data["paymentmethod"]], "status" => $data["status"], "rawstatus" => strtolower($data["status"])];
            }
            ...
            foreach ($attachments as $attachment) {
                ...
                if ($filename) {
                    $attachmentsArray[$attachment->id] = ["filename" => $filename, "uploadDate" => $attachment->createdAt->toClientDateTimeFormat()];
                }
            }
            ...
            $i = 1;
            $tasks = [];
            for ($result = select_query("mod_projecttasks", "id,task,notes,adminid,created,duedate,completed", ["projectid" => $projectid], "order", "ASC"); $data = mysql_fetch_assoc($result); $i++) {
                $tasks[$i] = $data;
                $tasks[$i]["adminname"] = $data["adminid"] ? get_query_val("tbladmins", "CONCAT(firstname,' ',lastname)", ["id" => $data["adminid"]]) : "0";
                ...
                $totaltasktime = 0;
                $result2 = select_query("mod_projecttimes", "", ["projectid" => $projectid, "taskid" => $data["id"]]);
                while ($data = mysql_fetch_array($result2)) {
                    ...
                    $tasks[$i]["times"][] = ["id" => $data["id"], "adminid" => $data["adminid"], "adminname" => get_query_val("tbladmins", "CONCAT(firstname,' ',lastname)", ["id" => $data["adminid"]]), "start" => $starttime, "end" => $endtime, "duration" => $totaltime];
                    if ($timerend) {

...
function widget_staffboard_overview($vars)
{
    ...
    $lastviews = get_query_val("tbladdonmodules", "value", ["module" => "staffboard", "setting" => "lastviewed"]);
    if ($lastviews) {
    ...
    if ($new) {
        insert_query("tbladdonmodules", ["module" => "staffboard", "setting" => "lastviewed", "value" => safe_serialize($lastviews)]);
    } else {
        update_query("tbladdonmodules", ["value" => safe_serialize($lastviews)], ["module" => "staffboard", "setting" => "lastviewed"]);
    }
    ...
    $content .= "</div>";
    return ["title" => $title, "content" => $content, "jquerycode" => NULL];
}

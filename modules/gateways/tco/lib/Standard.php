...
class Standard
{
    ...
    public function callback($params = [])
    {
        ...
        try {
            ...
            if (in_array($message_type, ["FRAUD_STATUS_CHANGED", "ORDER_CREATED", "RECURRING_INSTALLMENT_SUCCESS"])) {
                ...
                if ($message_type == "ORDER_CREATED" && $params["skipfraudcheck"]) {
                    ...
                    if ($message_type == "RECURRING_INSTALLMENT_SUCCESS") {
                        ...
                        if (!$invoiceid && !$serviceid) {
                            ...
                            if (!$invoiceid) {
                                logTransaction($params["paymentmethod"], array_merge(["InvoiceLookup" => "No invoice match found for Service ID " . $serviceid . " or Subscription ID"], $_POST), "Recurring Error");
                            }
                        }
                        ...
                        if ($serviceid && $transid) {
                            \WHMCS\Database\Capsule::table("tblhosting")->where("id", $serviceid)->update(["subscriptionid" => $transid]);
                        }
                    } else {
    ...
    public function clientCallback($params = [])
    {
        if ($params["secretword"]) {

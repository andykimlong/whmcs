...
class Hipchat implements \WHMCS\Module\Contracts\NotificationModuleInterface
{
    ...
    {
        return ["api_token" => ["FriendlyName" => "API Token", "Type" => "text", "Description" => "Requires a HipChat API V2 User Token. To create one, navigate to Account Settings > API Access. The token requires the View Room and Send Notification scopes."], "api_url" => ["FriendlyName" => "API URL", "Type" => "text", "Placeholder" => HOSTED_API_URL, "Description" => "If using HipChat Server Edition, enter the URL to your self-hosted Hipchat Instance."]];
    }
    ...
    {
        return ["room" => ["FriendlyName" => "Room", "Type" => "dynamic", "Description" => "Select the desired room for a notification.", "Required" => true], "message" => ["FriendlyName" => "Customise Message", "Type" => "text", "Description" => "Allows you to customise the primary display message shown in the notification."], "notify" => ["FriendlyName" => "User Notification", "Type" => "yesno", "Description" => "This can include playing a sound, a mobile notification and more based on your notification settings."]];
    }
    public function getDynamicField($fieldName, $settings)
    {
        if ($fieldName == "room") {
            ...
            foreach ($response["items"] as $room) {
                $rooms[] = ["id" => $room["id"], "name" => $room["name"], "description" => "Room ID"];
            }
            return ["values" => $rooms];
        } else {
            return [];
    ...
    public function sendNotification(\WHMCS\Notification\Contracts\NotificationInterface $notification, $moduleSettings, $notificationSettings)
    {
        ...
        $message = (new Message())->message(\WHMCS\Input\Sanitize::decode($notification->getTitle()) . "<br>" . $messageBody . "<br>" . "<a href=\"" . $notification->getUrl() . "\">" . $notification->getUrl() . "</a>")->notify((int) $notificationSettings["notify"])->card($card);

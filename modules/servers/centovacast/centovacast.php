...
{
    return ["DisplayName" => "Centova Cast", "APIVersion" => "1.0", "DefaultNonSSLPort" => "80", "DefaultSSLPort" => "443"];
}
function centovacast_ConfigOptions()
{
    $configarray = ["Account template name" => ["Type" => "text", "Size" => "20", "Description" => "<br />(create this in Centova Cast)"], "Max listeners" => ["Type" => "text", "Size" => "5", "Description" => "(simultaneous)<br />(blank to use template setting)"], "Max bit rate" => ["Type" => "dropdown", "Options" => ",8,16,20,24,32,40,48,56,64,80,96,112,128,160,192,224,256,320", "Description" => "kbps<br />(blank to use template setting)"], "Data transfer limit" => ["Type" => "text", "Size" => "5", "Description" => "MB/month<br />(blank to use template setting)"], "Disk quota" => ["Type" => "text", "Size" => "5", "Description" => "MB<br />(blank to use template setting)"], "Start server" => ["Type" => "dropdown", "Options" => "no,yes", "Description" => "<br>(only used if source is disabled)"], "Mount point limit" => ["Type" => "text", "Size" => "5", "Description" => "<br />(blank to use template setting)"], "Port 80 proxy" => ["Type" => "dropdown", "Options" => ",Enabled,Disabled", "Description" => "<br />(blank to use template setting)"], "AutoDJ support" => ["Type" => "dropdown", "Options" => ",Enabled,Disabled", "Description" => "<br />(blank to use template setting)"], "Max accounts" => ["Type" => "text", "Size" => "5", "Description" => "(resellers only)<br />(blank to use template setting)"], "Max bandwidth" => ["Type" => "text", "Size" => "5", "Description" => "kbps (resellers only)<br />(blank to use template setting)"]];
    return $configarray;
}
...
function centovacast_GetAPIArgs($params, $arguments)
{
    ...
    $addonmap = [CC_TXT_MAXCLIENTS => "maxclients", CC_TXT_MAXBITRATE => "maxbitrate", CC_TXT_XFERLIMIT => "transferlimit", CC_TXT_DISKQUOTA => "diskquota", CC_TXT_MAXBW => "resellerbandwidth", CC_TXT_MAXACCT => "resellerusers", CC_TXT_MOUNTLIMIT => "mountlimit"];
...
function centovacast_CreateAccount($params)
{
    ...
    logModuleCall("centovacast", "create", $system->raw_request, $system->raw_response, NULL, [$serverpassword]);
    if ($system->success) {
            ...
            if ($packageId) {
                ...
                foreach ($customFields as $customField) {
                    ...
                    if (array_key_exists($fieldName, $account)) {
                        ...
                        $customFieldValue = $modelToUse->customFieldValues()->firstOrNew(["fieldid" => $fieldId, "relid" => $id]);
...
function centovacast_ChangePackage($params)
{
    ...
    logModuleCall("centovacast", "changepackage", $server->raw_request, $server->raw_response, NULL, [$serverpassword]);
...
function centovacast_TerminateAccount($params)
{
    ...
    logModuleCall("centovacast", "terminate", $system->raw_request, $system->raw_response, NULL, [$serverpassword]);
    return $system->success ? "success" : $system->error;
}
function centovacast_SuspendAccount($params)
{
    ...
    $ccurl = centovacast_getccurl($params);
    $system = new CCSystemAPIClient($ccurl);
    $arguments = ["username" => $username, "status" => "disabled"];
    $system->call("setstatus", $serverpassword, $arguments);
    logModuleCall("centovacast", "suspend", $system->raw_request, $system->raw_response, NULL, [$serverpassword]);
    return $system->success ? "success" : $system->error;
}
function centovacast_UnsuspendAccount($params)
{
    ...
    $ccurl = centovacast_getccurl($params);
    $system = new CCSystemAPIClient($ccurl);
    $arguments = ["username" => $username, "status" => "enabled"];
    $system->call("setstatus", $serverpassword, $arguments);
    logModuleCall("centovacast", "unsuspend", $system->raw_request, $system->raw_response, NULL, [$serverpassword]);
    return $system->success ? "success" : $system->error;
}
function centovacast_ChangePassword($params)
{
    ...
    $server->call("reconfigure", $username, $serverpassword, $account);
    logModuleCall("centovacast", "changepassword", $server->raw_request, $server->raw_response, NULL, [$serverpassword]);
    return $server->success ? "success" : $server->error;
}
function centovacast_AdminCustomButtonArray()
{
    return ["Start Stream" => "StartStream", "Stop Stream" => "StopStream", "Restart Stream" => "RestartStream"];
}
function centovacast_ClientArea($params)
{
    ...
    if (file_exists($fn)) {
        ...
        $details = preg_replace("/<!--[\\s\\S]*?-->/", "", str_replace(["[CCURL]", "[USERNAME]", "[TIME]", "[AUTH]"], [$ccurl, preg_replace("/[^a-z0-9_]+/i", "", $username), $time, $authtoken], file_get_contents($fn)));
    } else {
        $details = "";
    }
    return $form . $details;
...
function centovacast_SetState($params, $newstate)
{
    ...
    logModuleCall("centovacast", "setstate", $server->raw_request, $server->raw_response, NULL, [$serverpassword]);
    return $server->success ? "success" : $server->error;
}
...
function centovacast_UsageUpdate($params)
{
    ...
    logModuleCall("centovacast", "usageupdate", $system->raw_request, $system->raw_response, NULL, [$serverpassword]);
    if (!$system->success) {
        return $system->error;
    }
    ...
    $hostingAccounts = WHMCS\Service\Service::where("server", "=", $serverid)->whereIn("domainstatus", ["Active", "Suspended"])->get();
    $hostingAddonAccounts = WHMCS\Service\Addon::where("server", "=", $serverid)->whereIn("status", ["Active", "Suspended"])->get();
    foreach ($accounts as $k => $values) {
        $updateData = ["diskusage" => $values["diskusage"], "disklimit" => max(0, $values["diskquota"]), "bwusage" => $values["transferusage"], "bwlimit" => max(0, $values["transferlimit"]), "lastupdate" => WHMCS\Carbon::now()->toDateTimeString()];

...
abstract class CCBaseAPIClient
{
    ...
    {
        $ctx = stream_context_create(["http" => ["method" => "POST", "user_agent" => "Centova Cast PHP API Client", "header" => "Connection: close\r\nContent-Length: " . strlen($postdata) . "\r\n", "max_redirects" => "0", "ignore_errors" => "1", "content" => $postdata]]);
        $this->http_request_error = "";
        set_error_handler([$this, "handle_http_error"]);
    ...
    protected function api_request($packet)
    {
        $url = $this->ccurl;
        $apiscript = "api.php";
        if (substr($url, -1 * strlen($apiscript) - 1) != "/" . $apiscript) {
            if (substr($url, -1) != "/") {
                $url .= "/";
            }
            $url .= $apiscript;
...
class CCServerAPIClient extends CCBaseAPIClient
{
    ...
    protected function build_argument_payload($functionargs)
    {
        ...
        $arguments = array_merge(["username" => $username, "password" => $password], $arguments);
...
class CCAPIXML
{
    ...
    private function get_first_tag($xml)
    {
        if (preg_match("/<\\s*([a-zA-Z0-9_:\\.-]+)([^\\>]*?)(\\/)?\\s*>/", $xml, $matches, PREG_OFFSET_CAPTURE)) {
            ...
            return [$tagoffset, $taglength, $tagname, $tagattr, $tagnocontent];
        }
        return false;
    }
    private function get_xml_tag_contents($xml, $tag)
    {
        ...
        $endoffset = $tagoffset;
        return [$endoffset + $taglength, substr($xml, $beginoffset, $endoffset - $beginoffset)];
    }
}

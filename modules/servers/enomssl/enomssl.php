...
{
    return ["DisplayName" => "eNom SSL Certificates", "APIVersion" => "1.0", "RequiresServer" => false, "AutoGenerateUsernameAndPassword" => false];
}
function enomssl_ConfigOptions($params)
{
    ...
    $configarray = ["Username" => ["Type" => "text", "Size" => "25"], "Password" => ["Type" => "password", "Size" => "25"], "Certificate Type" => ["Type" => "dropdown", "Options" => $certtypelist], "Years" => ["Type" => "dropdown", "Options" => "1"], "Test Mode" => ["Type" => "yesno"]];
    return $configarray;
}
function enomssl_CreateAccount($params)
{
    ...
    if (1 < $certyears) {
        return "Multi year SSL Certificates are no longer supported. Remove the years configurable option and try again.";
    }
    ...
    $sslOrder = WHMCS\Service\Ssl::firstOrNew(["userid" => $params["clientsdetails"]["userid"], "serviceid" => $params["serviceid"], "addon_id" => $params["addonId"], "module" => "enomssl"]);
...
{
    $buttonarray = ["Resend Configuration Email" => "resend"];
    return $buttonarray;
}
...
function enomssl_AdminServicesTabFields($params)
{
    ...
    $fieldsarray = ["Enom Order ID" => $remoteid, "SSL Configuration Status" => $status];
    return $fieldsarray;
}
function enomssl_SSLStepOne($params)
{
    ...
    if (stristr($certtype, "-ucc-")) {
        $values["additionalfields"]["Additional Certificate Configuration"] = ["ucc_domains" => ["FriendlyName" => "Domain Name List", "Type" => "textarea", "Rows" => "4", "Description" => "A list of Domains for this Certificate. One per line. Include the CSR Domain.", "Required" => true], "ucc_emails" => ["FriendlyName" => "Approval Emails List", "Type" => "textarea", "Rows" => "4", "Description" => "A list of Approval Emails for this Certificate. One per line for each domain.", "Required" => true]];
    }
    return $values;
}
function enomssl_SSLStepTwo($params)
{
    ...
    $postfields["CSR"] = $csr;
    $contacttypes = ["Admin", "Tech", "Billing"];
...
function enomssl_call($fields, $testmode = "")
{
    ...
    logModuleCall("enomssl", $fields["command"], $fields, $result, "", [$fields["uid"], $fields["pw"]]);
    return $result;
}

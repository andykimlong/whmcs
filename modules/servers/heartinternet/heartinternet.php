...
{
    return ["DisplayName" => "Heart Internet", "APIVersion" => "1.0"];
}
function heartinternet_ConfigOptions()
{
    $configarray = ["Test Mode" => ["Type" => "yesno", "Description" => "Tick to enable test mode"], "Package ID" => ["Type" => "text", "Size" => "25"]];
    return $configarray;
}
...
function heartinternet_ChangePassword($params)
{
    ...
    if (trim($xmldata["epp"]["response"]["result"]["attr"]["code"]) != "1000") {
        ...
        if ($newpassword != $oldpassword) {
            $params["model"]->serviceProperties->save(["password" => $newpassword]);
        }
        $result = "success";
    }
    return $result;
...
{
    while (!class_exists("HeartInternet_API")) {
    ...
    $objects = ["http://www.heartinternet.co.uk/whapi/null-1.1", "http://www.heartinternet.co.uk/whapi/package-1.4"];
    $extensions = ["http://www.heartinternet.co.uk/whapi/ext-package-1.1", "http://www.heartinternet.co.uk/whapi/ext-whapi-1.0"];
    try {

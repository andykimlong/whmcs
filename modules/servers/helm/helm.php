...
{
    return ["DisplayName" => "Helm", "APIVersion" => "1.0"];
}
function helm_ConfigOptions()
{
    $configarray = ["Package ID" => ["Type" => "text", "Size" => "25"], "Package Name" => ["Type" => "text", "Size" => "25"], "Reseller Plan ID" => ["Type" => "text", "Size" => "25", "Description" => "Only if reseller account"], "Helm Username" => ["Type" => "yesno", "Description" => "Let Helm Generate Account User ID"]];
    return $configarray;
}
...
function helm_CreateAccount($params)
{
    ...
    if ($data["RESULTS"]["RESULTCODE"] == "0") {
        $params["model"]->serviceProperties->save(["username" => $data["RESULTS"]["RESULTDATA"]]);
        $url = $params["serverhttpprefix"] . "://" . $params["serverip"] . "/billing_api.asp";

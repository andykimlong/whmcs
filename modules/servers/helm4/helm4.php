...
{
    return ["DisplayName" => "Helm 4", "APIVersion" => "1.0", "DefaultNonSSLPort" => "8086"];
}
function helm4_ConfigOptions()
{
    $configarray = ["Account Role ID" => ["Type" => "text", "Size" => "5", "Description" => ""], "Plan ID" => ["Type" => "text", "Size" => "5", "Description" => ""]];
    return $configarray;
}
...
{
    $fields = ["AccountName" => $params["serverusername"], "Username" => $params["serverusername"], "Password" => $params["serverpassword"], "action" => "GetAccountByName", "UserAccountName" => $params["clientsdetails"]["email"], "IncludeChildren" => "false"];
    ...
    if ($helmaccountid) {
        ...
        $data = get_query_vals("tblhosting", "username,password", ["username" => $helmaccountusername]);
        if (!$data) {
            $addons = WHMCS\Service\Addon::whereHas("customFieldValues.customField", function ($query) {
                $query->whereIn("fieldname", ["Username", "Password"]);
                ...
                foreach ($addon->customFieldValues as $customFieldValue) {
                    if ($customFieldValue->customField) {
                        if ($updatedUsername && $updatedPassword) {
                            if ($updatedUsername && $updatedPassword) {
                                $params["model"]->serviceProperties->save(["username" => $updatedUsername, "password" => $updatedPassword]);
                            }
                        } else {
            ...
            $params["model"]->serviceProperties->save(["username" => $data["username"], "password" => $data["password"]]);
        }
    }
    if (!$helmaccountid) {
    ...
    $fields = ["AccountName" => $params["serverusername"], "Username" => $params["serverusername"], "Password" => $params["serverpassword"], "action" => "AddPackage", "UserAccountID" => $helmaccountid, "PlanID" => $params["configoption2"], "PackageName" => $params["domain"], "Quantity" => "1"];
    $result = helm4_connect($params, $fields);
    $resultcode = $result["RESULTS"]["RESULTCODE"];
    $resultdescription = $result["RESULTS"]["RESULTDESCRIPTION"];
    if ($resultcode != "0") {
    ...
    $fields = ["AccountName" => $params["serverusername"], "Username" => $params["serverusername"], "Password" => $params["serverpassword"], "action" => "AddDomainToPackage", "UserAccountID" => $helmaccountid, "PackageID" => $helmpackageid, "DomainName" => $params["domain"], "IsPark" => "false"];
    $result = helm4_connect($params, $fields);
    $resultcode = $result["RESULTS"]["RESULTCODE"];
    $resultdescription = $result["RESULTS"]["RESULTDESCRIPTION"];
    if ($resultcode != "0") {
...
function helm4_SuspendAccount($params)
{
    ...
    $result = helm4_connect($params, $fields);
    $helmaccountid = $result["RESULTS"]["RESULTDATA"]["RECORD"]["ACCOUNTID"];
    $fields = ["AccountName" => $params["serverusername"], "Username" => $params["serverusername"], "Password" => $params["serverpassword"], "action" => "SuspendAccount", "UserAccountId" => $helmaccountid, "IncludeChildren" => "false"];
    $result = helm4_connect($params, $fields);
    $resultcode = $result["RESULTS"]["RESULTCODE"];
    $resultdescription = $result["RESULTS"]["RESULTDESCRIPTION"];
    if ($resultcode != "0") {
...
function helm4_UnsuspendAccount($params)
{
    ...
    $result = helm4_connect($params, $fields);
    $helmaccountid = $result["RESULTS"]["RESULTDATA"]["RECORD"]["ACCOUNTID"];
    $fields = ["AccountName" => $params["serverusername"], "Username" => $params["serverusername"], "Password" => $params["serverpassword"], "action" => "UnsuspendAccount", "UserAccountId" => $helmaccountid, "IncludeChildren" => "false"];
    $result = helm4_connect($params, $fields);
    $resultcode = $result["RESULTS"]["RESULTCODE"];
    $resultdescription = $result["RESULTS"]["RESULTDESCRIPTION"];
    if ($resultcode != "0") {
...
function helm4_TerminateAccount($params)
{
    ...
    $result = helm4_connect($params, $fields);
    $helmaccountid = $result["RESULTS"]["RESULTDATA"]["RECORD"]["ACCOUNTID"];
    $fields = ["AccountName" => $params["serverusername"], "Username" => $params["serverusername"], "Password" => $params["serverpassword"], "action" => "GetPackages", "UserAccountId" => $helmaccountid];
    $result = helm4_connect($params, $fields);
    $resultcode = $result["RESULTS"]["RESULTCODE"];
    $resultdescription = $result["RESULTS"]["RESULTDESCRIPTION"];
    if ($resultcode != "0") {
    ...
    $fields = ["AccountName" => $params["serverusername"], "Username" => $params["serverusername"], "Password" => $params["serverpassword"], "action" => "GetDomainsByPackageID", "UserAccountId" => $helmaccountid, "PackageID" => $helmpackageid];
    $result = helm4_connect($params, $fields);
    $resultcode = $result["RESULTS"]["RESULTCODE"];
    $resultdescription = $result["RESULTS"]["RESULTDESCRIPTION"];
    if ($resultcode != "0") {
    ...
    foreach ($output as $data) {
        ...
        $data = $data["RECORD"];
        if ($data) {
            $fields = ["AccountName" => $params["serverusername"], "Username" => $params["serverusername"], "Password" => $params["serverpassword"], "action" => "DeleteDomain", "DomainID" => $data["DOMAINID"]];
            $result = helm4_connect($params, $fields);
        }
    }
    $fields = ["AccountName" => $params["serverusername"], "Username" => $params["serverusername"], "Password" => $params["serverpassword"], "action" => "DeletePackage", "UserAccountId" => $helmaccountid, "PackageID" => $helmpackageid];
    $result = helm4_connect($params, $fields);
    $resultcode = $result["RESULTS"]["RESULTCODE"];
    $resultdescription = $result["RESULTS"]["RESULTDESCRIPTION"];
    if ($resultcode != "0") {
...
function helm4_ChangePackage($params)
{
    ...
    $helmpackageid = $helmpackagesarray[$params["domain"]]["PACKAGEID"];
    $fields = ["AccountName" => $params["serverusername"], "Username" => $params["serverusername"], "Password" => $params["serverpassword"], "action" => "UpgradePackage", "UserAccountId" => $helmaccountid, "PackageID" => $helmpackageid, "NewPlanID" => $params["configoption2"]];
    $result = helm4_connect($params, $fields);
    $resultcode = $result["RESULTS"]["RESULTCODE"];
    $resultdescription = $result["RESULTS"]["RESULTDESCRIPTION"];
    if ($resultcode != "0") {
...
function helm4_connect($params, $fields)
{
    ...
    logModuleCall("helm4", $fields["action"], $fields, $data, $result, [$fields["AccountName"], $fields["Username"], $fields["Password"]]);
    $result["raw"] = $data;
    return $result;
}

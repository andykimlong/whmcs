...
{
    return ["DisplayName" => "InterWorx", "APIVersion" => "1.0", "DefaultNonSSLPort" => "2080", "DefaultSSLPort" => "2443"];
}
function interworx_ConfigOptions()
{
    $configarray = ["Package Name" => ["Type" => "text", "Size" => "25"], "Theme" => ["Type" => "text", "Size" => "25"], "Disk & BW Overselling" => ["Type" => "yesno", "Description" => "If reseller, tick to allow"]];
    return $configarray;
}
...
function interworx__getSoapClient($params)
{
    if ($params["serverhostname"]) {
        ...
        $soapParams = ["stream_context" => stream_context_create($opts)];
    }
...
function interworx_TerminateAccount($params)
{
    ...
    if ($params["type"] == "reselleraccount") {
        ...
        $api_controller = "/nodeworx/reseller";
        $action = "delete";
        $input = ["reseller_id" => $resellerid];
    } else {
        $api_controller = "/nodeworx/siteworx";
        $action = "delete";
        $input = ["domain" => $params["domain"], "confirm_action" => "1"];
    }
    $client = interworx__getsoapclient($params);
    $result = $client->route($key, $api_controller, $action, $input);
    logModuleCall("interworx", $action, $input, $result);
...
function interworx_UsageUpdate($params)
{
    ...
    $services = WHMCS\Service\Service::whereIn("domainstatus", ["Active", "Suspended"])->get();
    $addons = WHMCS\Service\Addon::whereHas("customFieldValues.customField", function ($query) {
        $query->where("fieldname", "Domain");
    })->with("customFieldValues", "customFieldValues.customField")->whereIn("status", ["Active", "Suspended"])->get();
    foreach ($domainsdata as $data) {
        ...
        if ($model) {
            $model->serviceProperties->save(["diskusage" => $storage_used, "disklimit" => $storage, "bwusage" => $bandwidth_used, "bwlimit" => $bandwidth, "lastupdate" => WHMCS\Carbon::now()->toDateTimeString()]);
        }
    }
}
function interworx_SuspendAccount($params)
{
    ...
    if ($params["type"] == "reselleraccount") {
        ...
        $api_controller = "/nodeworx/reseller";
        $action = "edit";
        $input = ["reseller_id" => $resellerid, "status" => "inactive"];
    } else {
        $api_controller = "/nodeworx/siteworx";
        $action = "edit";
        $input = ["domain" => $params["domain"], "status" => "0"];
    }
    $client = interworx__getsoapclient($params);
    $result = $client->route($key, $api_controller, $action, $input);
    logModuleCall("interworx", $action, $input, $result);
...
function interworx_UnsuspendAccount($params)
{
    ...
    if ($params["type"] == "reselleraccount") {
        ...
        $api_controller = "/nodeworx/reseller";
        $action = "edit";
        $input = ["reseller_id" => $resellerid, "status" => "active"];
    } else {
        $api_controller = "/nodeworx/siteworx";
        $action = "edit";
        $input = ["domain" => $params["domain"], "status" => "1"];
    }
    $client = interworx__getsoapclient($params);
    $result = $client->route($key, $api_controller, $action, $input);
    logModuleCall("interworx", $action, $input, $result);
...
function interworx_ChangePassword($params)
{
    ...
    if ($params["type"] == "reselleraccount") {
        ...
        $api_controller = "/nodeworx/reseller";
        $action = "edit";
        $input = ["reseller_id" => $resellerid, "password" => $params["password"], "confirm_password" => $params["password"]];
    } else {
        $api_controller = "/nodeworx/siteworx";
        $action = "edit";
        $input = ["domain" => $params["domain"], "password" => $params["password"], "confirm_password" => $params["password"]];
    }
    $client = interworx__getsoapclient($params);
    $result = $client->route($key, $api_controller, $action, $input);
    logModuleCall("interworx", $action, $input, $result);
...
function interworx_ChangePackage($params)
{
    ...
    if ($params["type"] == "reselleraccount") {
        ...
        $api_controller = "/nodeworx/reseller";
        $action = "edit";
        $input = ["reseller_id" => $resellerid, "package_template" => $params["configoption1"], "RSL_OPT_OVERSELL_STORAGE" => $overselling, "RSL_OPT_OVERSELL_BANDWIDTH" => $overselling];
    } else {
        $api_controller = "/nodeworx/siteworx";
        $action = "edit";
        $input = ["domain" => $params["domain"], "package_template" => $params["configoption1"]];
    }
    $client = interworx__getsoapclient($params);
    $result = $client->route($key, $api_controller, $action, $input);
    logModuleCall("interworx", $action, $input, $result);

...
{
    return ["DisplayName" => "ResellerClub SSL Certificates", "APIVersion" => "1.1", "RequiresServer" => false];
}
function resellerclubssl_ConfigOptions($params)
{
    ...
    if (!$customField) {
        $customField = new WHMCS\CustomField(["relid" => $pid, "type" => $customFieldType, "fieldName" => "Domain Name", "fieldType" => "text"]);
    ...
    $configarray = ["Reseller ID" => ["Type" => "text", "Size" => "20", "Description" => "Obtained from ResellerClub Settings > Personal Information > Primary Profile"], "API-Key" => ["Type" => "password", "Size" => "20", "Description" => "Your API Key. You can get this from the LogicBoxes Control Panel in Settings -> API"], "Certificate Type" => ["Type" => "dropdown", "Options" => $certificateTypes, "Description" => $certificateTypeDescription], "Test Mode" => ["Type" => "yesno"]];
    return $configarray;
}
function resellerclubssl_CreateAccount($params)
{
    ...
    updateService(["domain" => $domainName, "username" => "", "password" => ""]);
    ...
    $sslOrder = WHMCS\Service\Ssl::firstOrNew(["userid" => $params["clientsdetails"]["userid"], "serviceid" => $params["serviceid"], "addon_id" => $params["addonId"], "module" => "resellerclubssl"]);
    ...
    $sslOrder->save();
    sendMessage("SSL Certificate Configuration Required", $params["serviceid"], ["ssl_configuration_link" => $sslOrder->getConfigurationUrl()]);
    return "success";
}
...
{
    $buttonarray = ["Resend Configuration Email" => "resend", "Prepare for Reissue" => "Reissue"];
    return $buttonarray;
}
...
function resellerclubssl_AdminServicesTabFields($params)
{
    ...
    $fieldsarray = ["ResellerClub Order ID" => $remoteid, "SSL Configuration Status" => $status];
    return $fieldsarray;
}
function resellerclubssl_SSLStepOne($params)
{
    ...
    if ($params["remoteid"] && !strpos($params["certtype"], "|")) {
        ...
        if ($params["remoteid"]) {
            ...
            if ((int) $certificateData["actioncompleted"]) {
                $sslOrder->completionDate = WHMCS\Carbon::now()->toDateTimeString();
                $sslOrder->status = WHMCS\Service\Ssl::STATUS_COMPLETED;
            } else {
                $sslOrder->completionDate = "0000-00-00 00:00:00";
...
function resellerclubssl_SSLStepTwo($params)
{
    ...
    $approveremails = ["admin", "administrator", "hostmaster", "postmaster", "webmaster"];
...
function resellerclubssl_SSLStepThree($params)
{
    if (!strpos($params["certtype"], "|")) {
        ...
        if ($result["hashtable"]["entry"][0]["string"][1] != "success") {
            return ["error" => $result["hashtable"]["entry"][1]["string"][1]];
        }
    } else {
        $postFields = [];
        $postFields["auth-userid"] = $params["configoption1"];
        ...
        if ($certificateData["response"]["status"] == "ERROR") {
            ...
            return ["error" => $certificateData["response"]["message"]];
        }
...
function resellerclubssl_SendCommand($command, $type, $postfields, $params, $method)
{
    ...
    curl_close($ch);
    logModuleCall("logicboxes", $command, $postfields, $data, $result, [$params["configoption1"], $params["configoption2"]]);
...
function resellerclubssl_getSSLPlans($params)
{
    ...
    if ($result["status"] == "ERROR") {
        return [str_replace(",", "", $result["message"])];
    }
    if ($result["response"]["status"] == "ERROR") {
        if (!$result["response"]["message"]) {
            $result["response"]["message"] = $result["response"]["error"];
        }
        return [str_replace(",", "", $result["response"]["message"])];
    }
...
function resellerclubssl_sendJsonCommand($command, $type, $postFields, $params, $method, $noDecode = false)
{
    ...
    switch ($method) {
        ...
        default:
            ...
            curl_close($ch);
            logModuleCall("logicboxes", $type . "/" . $command, $postFields, $data, $result, [$params["configoption1"], $params["configoption2"]]);
            if ($result["response"]["message"] == "An unexpected error has occurred") {
                $result["response"]["message"] = "Login Failure or Unexpected Error";
            }
            return $result;

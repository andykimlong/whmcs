...
{
    return ["DisplayName" => "SCPanel", "APIVersion" => "1.0", "DefaultNonSSLPort" => "2086", "DefaultSSLPort" => "2087"];
}
function scpanel_ConfigOptions()
{
    $configarray = ["Super User" => ["Type" => "text", "Size" => "25"], "Super User Password" => ["Type" => "text", "Size" => "25"], "OS" => ["Type" => "dropdown", "Options" => "linux,freebsd4,freebsd5,solaris,macosx"], "Max Users" => ["Type" => "text", "Size" => "5"], "Next Port" => ["Type" => "text", "Size" => "5"], "Max Traffic" => ["Type" => "text", "Size" => "5", "Description" => "MB (enter unmetered for unlimited)"], "Traffic Abuse" => ["Type" => "yesno", "Description" => "If ticked, stream will auto-suspend on traffic abuse"], "Max Bit Rate" => ["Type" => "dropdown", "Options" => "8,16,20,24,32,48,56,64,80,96,112,128,160,192,224,256,320,384,512,768,1024"], "Bit Rate Abuse" => ["Type" => "yesno", "Description" => "If ticked, stream will auto-suspend on bitrate abuse"], "On Demand" => ["Type" => "yesno", "Description" => "If ticked, the customer will be able to upload and use on-demand content"], "Proxy" => ["Type" => "yesno", "Description" => "If ticked, the customer will be able to use SCPanel Proxy to stream through port 80"], "WHM Package Name" => ["Type" => "text", "Size" => "25"], "Intro Backup Max Size" => ["Type" => "text", "Size" => "5", "Description" => "MB (leave blank for default)"], "On Demand Max Size" => ["Type" => "text", "Size" => "5", "Description" => "MB (leave blank for default)"], "Use Port Prefix Domain" => ["Type" => "text", "Size" => "30", "Description" => "To use port.domain.com for SCPanel accounts, enter the .domain.com ending here"]];
    return $configarray;
}
...
function scpanel_CreateAccount($params)
{
    ...
    $params["model"]->serviceProperties->save(["domain" => $params["domain"], "username" => $params["username"]]);
    ...
    $header = ["Authorization: Basic " . $sAuth];
    ...
    if ($result == "success") {
        ...
        update_query("tblproducts", ["configoption5" => "+=2"], ["id" => $params["packageid"]]);
    }
    return $result;
}
...
{
    $buttonarray = ["Start" => "start", "Stop" => "stop"];
    return $buttonarray;
}

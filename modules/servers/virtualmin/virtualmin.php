...
{
    $configarray = ["Template Name" => ["Type" => "text", "Size" => "30"], "Plan Name" => ["Type" => "text", "Size" => "30"], "Dedicated IP" => ["Type" => "yesno", "Description" => "Tick to auto assign next available dedicated IP"]];
    return $configarray;
}
...
function virtualmin_UsageUpdate($params)
{
    ...
    foreach ($dataArray as $values) {
        ...
        if ($domain) {
            $getScalarValue = function ($key, $data) {
            ...
            if ($model) {
                $model->serviceProperties->save(["diskusage" => $diskusage, "disklimit" => $disklimit, "bwusage" => $bwused, "bwlimit" => $bwlimit, "lastupdate" => WHMCS\Carbon::now()->toDateTimeString()]);
            }
        }
    }
}
function virtualmin_req($params, $postfields, $rawdata = false)
{
    ...
    $url = $http . "://" . $domain . ":" . $port . "/virtual-server/remote.cgi?";
    $fieldstring = "";

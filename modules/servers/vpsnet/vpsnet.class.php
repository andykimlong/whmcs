...
class VPSNET
{
    ...
    public static function getInstance($username = "", $_auth_api_key = "", $proxy = "")
    {
        if (!isset($instance)) {
            ...
            if (strlen($username) == 0 || strlen($_auth_api_key) == 0) {
                throw new Exception("A Username and/or API Key has not yet been set up in Configuration \\(<i class=\"fa fa-wrench\" aria-hidden=\"true\"></i>\\) > System Settings > Servers.");
            }
    ...
    public function sendGETRequest()
    {
        ...
        curl_setopt($this->ch, CURLOPT_HTTPHEADER, ["Content-Length: 0", "Content-Type: application/json", "Accept: application/json"]);
    ...
    public function sendPUTRequest($data)
    {
        ...
        curl_setopt($this->ch, CURLOPT_HTTPHEADER, ["Content-Length: " . strlen($json_data), "Content-Type: application/json", "Accept: application/json"]);
        $rtn = $this->sendRequest();
...
class VirtualMachine
{
    ...
    public function update()
    {
        ...
        $api->setAPIResource("virtual_machines/" . $this->id);
        $_virtual_machine_keys = ["label" => "", "backups_enabled" => "", "slices_required" => ""];
        $vm = $this;

...
{
    return ["DisplayName" => "VPS.net", "APIVersion" => "1.0"];
}
function vpsnet_ConfigOptions($params)
{
    ...
    if (!$creds["id"]) {
        return ["Error" => ["Type" => "x", "Description" => "No VPS.Net Server Config found in Configuration (<i class=\"fa fa-wrench\" aria-hidden=\"true\"></i>) > System Settings > Servers"]];
    }
    ...
    if ($verifyauth["success"] != "1") {
        ...
        if ($verifyautherror) {
            return ["Error" => ["Type" => "x", "Description" => $verifyautherror]];
        }
        return ["Error" => ["Type" => "FailedAuth", "Description" => "Unable to authenticate with Username and Access Hash. Please check Server Config found in Configuration \\(<i class=\"fa fa-wrench\" aria-hidden=\"true\"></i>\\) > System Settings > Servers"]];
    } else {
        ...
        $configarray = ["Number of Nodes" => ["Type" => "text", "Size" => "5"], "Cloud/Template" => ["Type" => "dropdown", "Options" => $cloudtemplate], "Enable Backups" => ["Type" => "yesno", "Description" => "Tick to enable backups"], "Rsync Backups" => ["Type" => "yesno", "Description" => "Tick to enable"], "R1Soft Backups" => ["Type" => "yesno", "Description" => "Tick to enable"], "" => ["Type" => "x", "Description" => ""]];
        return $configarray;
    }
}
function vpsnet_CreateAccount($params)
{
    ...
    $result = vpsnet_call($params, "", "", "", "virtual_machines", ["virtual_machine" => $postfields]);
    if ($result["success"] == "1") {
        ...
        $params["model"]->serviceProperties->save(["dedicatedip" => $ip, "password" => $password]);
        insert_query("mod_vpsnet", ["relid" => $params["serviceid"], "addon_id" => (int) $params["addonId"], "setting" => "netid", "value" => $netid]);
        insert_query("mod_vpsnet", ["relid" => $params["serviceid"], "addon_id" => (int) $params["addonId"], "setting" => "slices", "value" => $initialNodes]);
        return "success";
    }
    if (is_array($result)) {
...
{
    $netid = get_query_val("mod_vpsnet", "value", ["relid" => $params["serviceid"], "addon_id" => (int) $params["addonId"], "setting" => "netid"]);
    if (!$netid) {
        return "No VM Found";
    }
...
function vpsnet_TerminateAccount($params)
{
    ...
    if ($rtn["success"]) {
        delete_query("mod_vpsnet", ["relid" => $params["serviceid"], "addon_id" => (int) $params["addonId"]]);
    } else {
        return $rtn["errors"];
    }
}
...
{
    $creds = vpsnet_GetCredentials();
    $result = select_query("mod_vpsnet", "", ["relid" => $params["serviceid"], "addon_id" => (int) $params["addonId"]]);
    ...
    $postfields["r1_soft_backups_enabled"] = $r1softbackups;
    $postfields["slices_required"] = $initialNodes;
    $result = vpsnet_call($params, "", $netid, "PUT", "virtual_machines", ["virtual_machine" => $postfields]);
    if ($result["success"]) {
        update_query("mod_vpsnet", ["value" => $netid], ["relid" => $params["serviceid"], "addon_id" => (int) $params["addonId"], "setting" => "netid"]);
        update_query("mod_vpsnet", ["value" => $initialNodes], ["relid" => $params["serviceid"], "addon_id" => (int) $params["addonId"], "setting" => "slices"]);
        return "success";
    }
    if (is_array($result)) {
        $errors = $result["errors"]["errors"];
...
{
    $buttonarray = ["Manage Backups" => "managebackups"];
    return $buttonarray;
}
...
function vpsnet_recover($params)
{
    ...
    $rtn = vpsnet_call($params, "power_on", $netid, "", "", ["mode" => "recovery"]);
    if ($rtn["success"]) {
...
function vpsnet_AdminServicesTabFields($params)
{
    ...
    if ($_REQUEST["bwgraphs"]) {
        ...
        foreach ($data as $d) {
            $datatable[] = "[\"" . date("Y-m-d H:i", strtotime($d["created_at"])) . "\"," . round($d["data_received"] / 1048576, 2) . "," . round($d["data_sent"] / 1048576, 2) . "]";
        }
        ...
        return ["Bandwidth Graphs" => $vpsinfo];
    } else {
        if ($_REQUEST["managebackups"]) {
            ...
            if ($_REQUEST["rsyncbackups"]) {
                ...
                return ["Rsync Backups Info" => $vpsinfo];
            }
            ...
            $vpsinfo .= "\n</div>\n</div>\n\n\n";
            return ["VPS Overview" => $vpsinfo];
        }
    }
}
function vpsnet_ClientAreaAllowedFunctions()
{
    return ["cpugraphs", "networkgraphs"];
}
function vpsnet_cpugraphs($params)
{
    global $_LANG;
    $pagearray = ["templatefile" => "cpugraphs", "breadcrumb" => " > <a href=\"" . $_SERVER["PHP_SELF"] . "#\">" . $_LANG["vpsnetcpugraphs"] . "</a>", "vars" => ["serviceid" => $params["serviceid"], "addonid" => (int) $params["addonId"]]];
    return $pagearray;
}
function vpsnet_networkgraphs($params)
{
    global $_LANG;
    $pagearray = ["templatefile" => "networkgraphs", "breadcrumb" => " > <a href=\"" . $_SERVER["PHP_SELF"] . "#\">" . $_LANG["vpsnetnetworkgraphs"] . "</a>", "vars" => ["serviceid" => $params["serviceid"], "addonid" => (int) $params["addonId"]]];
    return $pagearray;
}
function vpsnet_call($params, $action, $id, $reqtype = "", $type = "virtual_machines", $data = "", $nojsonencode = "")
{
    ...
    if (curl_error($ch)) {
        $rtn["errors"] = ["Curl Error: " . curl_errno($ch) . " - " . curl_error($ch)];
    }
    curl_close($ch);
...
{
    return get_query_vals("tblservers", "id,username,accesshash", ["type" => "vpsnet"]);
}
function vpsnet_ClientArea($params)
{
    ...
    if ($whmcs->get_req_var("managebackups")) {
        ...
        foreach ($rtn["response"] as $backup) {
            ...
            $lastupdated = strtotime($lastupdated);
            $lastupdated = date("F dS, Y H:i", $lastupdated);
            $backups[] = ["type" => $type, "state" => $state, "size" => $size, "lastupdated" => $lastupdated];
        }

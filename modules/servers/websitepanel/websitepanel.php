...
{
    return ["DisplayName" => "WebsitePanel", "APIVersion" => "1.0"];
}
function websitepanel_ConfigOptions()
{
    ...
    $configarray = ["Package Name" => ["Type" => "text", "Size" => "25", "Description" => $soap_check_msg], "Web Space Quota" => ["Type" => "text", "Size" => "5", "Description" => "MB"], "Bandwidth Limit" => ["Type" => "text", "Size" => "5", "Description" => "MB"], "PlanID" => ["Type" => "text", "Size" => "3", "Description" => " * DNP Hosting Plan ID"], "Parent SpaceID" => ["Type" => "text", "Size" => "3", "Description" => "* SpaceID that all accounts are created under"], "Enterprise Server Port" => ["Type" => "text", "Size" => "5", "Description" => "* Required"], "Different Portal URL" => ["Type" => "yesno", "Description" => "Tick if portal address is different to server address"], "Portal URL" => ["Type" => "text", "Size" => "25", "Description" => "Portal URL, with http://, no trailing slash"], "Send DNP Account Summary email" => ["Type" => "yesno", "Description" => "Tick to send DNP Account Summary"], "Send DNP Hosting Space Summary email" => ["Type" => "yesno", "Description" => "Tick to send Hosting Space Summary"], "Create Mail account" => ["Type" => "yesno", "Description" => "Tick to create mail account"], "Create FTP account" => ["Type" => "yesno", "Description" => "Tick to create FTP account"], "Temporary domain" => ["Type" => "yesno", "Description" => "Tick to create a temp domain"], "HTML email" => ["Type" => "yesno", "Description" => "Tick enable HTML email from DNP"], "Create Website" => ["Type" => "yesno", "Description" => "Tick to create Website"], "Count Bandwidth/Diskspace" => ["Type" => "yesno", "Description" => "Tick to update diskpace/bandwidth in WHMCS"]];
    return $configarray;
}
...
function websitepanel_call($params, $func, $param, $retdata = "")
{
    ...
    try {
        $client = new SoapClient($soapaddress, ["login" => $serverusername, "password" => $serverpassword]);
        $result = (int) $client->{$func}($param);
...
function websitepanel_SuspendAccount($params)
{
    ...
    $param = ["userId" => $wspuserid, "status" => "Suspended"];
...
function websitepanel_UnsuspendAccount($params)
{
    ...
    $param = ["userId" => $wspuserid, "status" => "Active"];
    $result = websitepanel_call($params, "ChangeUserStatus", $param);
    return $result;
}
function websitepanel_ChangePassword($params)
{
    ...
    $param = ["userId" => $wspuserid, "password" => $params["password"]];
...
function websitepanel_ChangePackage($params)
{
    ...
    $param = ["packageId" => websitepanel_getpackageid($params, $wspuserid), "statusId" => 1, "planId" => $params["configoption4"], "purchaseDate" => date("c"), "packageName" => $params["configoption1"], "packageComments" => ""];
...
{
    $serverip = $params["serverip"];
    $serveridquery = select_query("tblservers", "id", ["ipaddress" => $serverip]);
...
function websitepanel_UsageUpdate($params)
{
    ...
    })->with("customFieldValues", "customFieldValues.customField", "productAddon", "productAddon.moduleConfiguration")->where("server", "=", $params["serverid"])->whereIn("status", ["Active", "Suspended"])->get();
    foreach ($services as $service) {
        try {
            ...
            if ($service->product->moduleConfigOption16 == "on") {
                ...
                $service->serviceProperties->save(["diskusage" => $diskSpace, "disklimit" => $dsLimit, "bwusage" => $bandwidth, "bwlimit" => $bwLimit, "lastupdate" => WHMCS\Carbon::now()->toDateTimeString()]);
            }
        } catch (Exception $e) {
        }
    }
    foreach ($addons as $addon) {
        try {
            if ($addon->productAddon->moduleConfiguration->where("settingName", "configoption16")->first()->value == "on") {
                ...
                $bandwidth = websitepanel_getBandwidth($params, $packageId, $startDate);
                $diskSpace = websitepanel_getDiskspace($params, $packageId);
                $addon->serviceProperties->save(["diskusage" => $diskSpace, "disklimit" => $dsLimit, "bwusage" => $bandwidth, "bwlimit" => $bwLimit, "lastupdate" => WHMCS\Carbon::now()->toDateTimeString()]);
            }
        } catch (Exception $e) {
        }
    }
...
{
    $param = ["packageId" => $packageID, "startDate" => $startDate, "endDate" => date("Y-m-d", time())];

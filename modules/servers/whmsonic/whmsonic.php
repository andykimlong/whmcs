...
{
    return ["DisplayName" => "WHMSonic", "APIVersion" => "1.0", "DefaultNonSSLPort" => "2086", "DefaultSSLPort" => "2087"];
}
function whmsonic_ConfigOptions()
{
    $configarray = ["Client Type" => ["Type" => "dropdown", "Options" => "External,internal", "Description" => "Notice: If Internal selected, please set the custom form field under the Custom Fields link, the client must enter cPanel username on the order page, that is also mean the client has already cPanel hosting account on the server. This option will not setup cPanel account! It will setup only radio under the provided cpanel user account."], "Max Listeners Limit" => ["Type" => "text", "Size" => "3"], "Max BitRate Limit" => ["Type" => "dropdown", "Options" => "64,128,24,32,48,96,192,384"], "AutoDJ Feature" => ["Type" => "yesno", "Description" => "If yes, the user will access to AutoDJ features in their cPanel WHMSonic."], "Hosting Space" => ["Type" => "text", "Size" => "25", "Description" => "Hosting space is required by external clients only if autodj option is enabled to upload music files. Please enter a limit, ex: 100 = 100MB. Enter only numbers in this field."], "Bandwidth Limit" => ["Type" => "text", "Size" => "25"]];
    return $configarray;
}
function whmsonic_CreateAccount($params)
{
    ...
    $params["model"]->serviceProperties->save(["username" => $radiousername, "password" => $pass]);
...
function whmsonic_call($url, $auth, $data)
{
    ...
    if ($retval == "Complete") {
        ...
        if (strpos($retval, "<title>WHM Login</title>")) {

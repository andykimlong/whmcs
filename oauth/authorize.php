...
if (!$isLoginDeclinedRequest) {
    ...
    if (!Auth::user()) {
        WHMCS\Authentication\LoginHandler::validateAndSaveGotoRequest(WHMCS\Input\Sanitize::decode($_SERVER["REQUEST_URI"]));
        ...
        $ca->assign("incorrect", (int) $whmcs->get_req_var("incorrect"));
        ...
        $clientAccount = Auth::client();
        $previouslyAuthorized = false;
        $clientName = "";
        if (!$clientAccount) {
            $clientAccount = Auth::user()->ownedClients()->first();
        }
        if ($clientAccount) {

...
$selectOptions = [];
$selectOptions[] = ["id" => "client", "type" => "client", "descriptor" => "", "name" => $client->fullName, "companyname" => $client->companyName, "email" => $client->email, "address" => $client->address1 . " " . $client->state . " " . $client->postcode . " " . $client->countryName];
foreach ($client->contacts as $contact) {
    $selectOptions[] = ["id" => $contact->id, "type" => "contact", "descriptor" => "", "name" => $contact->fullName, "companyname" => $contact->companyName, "email" => $contact->email, "address" => $contact->address1 . " " . $contact->state . " " . $contact->postcode . " " . $contact->countryName];
}
$selectedId = $client->billingContactId ? $client->billingContactId : "client";
if (isset($payMethod)) {
    $contact = $payMethod->contact;
    $selectedId = !$contact instanceof WHMCS\User\Client ? $selectedId = $contact->id : "client";
}

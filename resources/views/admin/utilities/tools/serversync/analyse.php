...
echo AdminLang::trans("global.stepOfStep", [":step" => 2, ":steps" => 4]);
echo ": Analyse</h2>\n";
if ($error) {
...
if (!$error) {
    ...
    echo AdminLang::trans("global.stepOfStep", [":step" => 3, ":steps" => 4]);
        ...
        if ($item->getServicesCount() == 0) {
            ...
            foreach ($item->getMatches() as $serviceIterator => $service) {
                ...
                if ($item->hasMultipleMatches() && !$service->isTerminated() && 0 < $serviceIterator) {
                    ...
                    if (!$item->hasExactMatch()) {
                        $value = $item->getUniqueIdentifier() . "||" . $service->getId();
                        $checked = "";
    ...
    foreach ($services as $service) {
        $statuses = [WHMCS\Service\Status::TERMINATED, WHMCS\Service\Status::CANCELLED, WHMCS\Service\Status::FRAUD, WHMCS\Service\Status::PENDING];

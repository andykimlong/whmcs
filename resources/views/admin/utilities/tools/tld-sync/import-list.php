...
echo AdminLang::trans("domains.tldImport.importCountTlds", [":count" => "<span id=\"tldImportCount\">0</span>"]);
...
foreach ($categories as $category => $data) {
    if (count($data)) {
        ...
        if (0 < count($tldCurrencies)) {
            echo AdminLang::trans("domains.tldImport.additionalCurrencies", [":currencies" => implode(", ", $tldCurrencies)]);
        }
        echo "                        ";
        echo AdminLang::trans("domains.tldImport.defaultCurrency", [":currency" => $currency["code"]]);
        ...
        foreach ($data as $tld) {
            ...
            if ($isExistingTld) {
                ...
                echo AdminLang::trans($text, [":registrar" => $autoRegistrar]);
                echo "\"></i>\n                                    ";
            }
            echo "                                </td>\n                                <td class=\"text-center\">\n                                    <i class=\"fas fa-";
            ...
            if (!$currencies->has($tld->getCurrency())) {
                ...
                if (is_null($tld->getCurrency())) {
                    ...
                    echo "                                            ";
                    echo AdminLang::trans("domains.tldImport.noCurrency", [":currencyCode" => $tld->getCurrency() ?: "Unknown"]);
                    echo "                                        ";
                }
                echo "                                    </td>\n                                ";
            } else {
                echo "                                    ";
                foreach (["register", "renew", "transfer", "grace", "redemption"] as $type) {
                    if ($showGrace || $type != "grace") {
                        $tdClass = " " . $type . "-pricing";
                        $margins = [];
                        if ($isExistingTld && isset($pricing[$extension][$type]["margin"])) {
...
echo AdminLang::trans("domains.tldImport.importCountTlds", [":count" => "<span id=\"tldImportCount2\">0</span>"]);
echo "            </button>\n        </div>\n    </div>\n</form>\n";

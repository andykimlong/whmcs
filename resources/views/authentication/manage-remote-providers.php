...
foreach ($providers as $provider) {
    $this->insert("partials/settings-remote-providers", ["provider" => $provider]);
}
...
echo "');\n\n            } else {\n                showIntegrationProviderError(\$submittedFrm, '');\n            }\n        }).fail(function () {\n            showIntegrationProviderError(\$submittedFrm, '<strong>System Error.</strong> Please refresh the page and try again.');\n        });\n\n    });\n\n    /** Save & Activate **/\n    \$providerForms.submit(function(e) {\n        e.preventDefault();\n        var \$submittedFrm = \$(this),\n            \$provider = \$submittedFrm.data('provider'),\n            \$saveActivateBtn = \$submittedFrm.find('button[type=\"submit\"]'),\n            \$saveActivateLabel = \$saveActivateBtn.html(),\n            \$deactivateBtn = \$submittedFrm.find('button[type=\"reset\"]'),\n            \$errorMessage = '";
echo WHMCS\Input\Sanitize::escapeSingleQuotedString(AdminLang::trans("signIn.invalidDetails"));

...
foreach ($providerFields as $field) {
    ...
    if (strcasecmp($fieldName, "enabled") === 0) {
        ...
        $inputs[] = sprintf("<div class=\"form-group\"><label for=\"%s\">%s</label><input class=\"form-control\" type=\"%s\" name=\"%s\" id=\"%s\" %s /></div>", $inputId, $fieldDisplayName, $fieldType, $inputId, $inputId, $attrValue);
    }
}
...
echo AdminLang::trans("help.title");

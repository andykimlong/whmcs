...
if ($action == "accept") {
    if (!$agreetos && $CONFIG["EnableTOSAccept"]) {
        ...
        $validQuote = WHMCS\Database\Capsule::table("tblquotes")->where("id", $id)->where("userid", $_SESSION["uid"])->whereNotIn("stage", ["Draft", "Accepted"])->first();
        if ($validQuote) {
            update_query("tblquotes", ["stage" => "Accepted", "dateaccepted" => "now()"], ["id" => $id]);
            logActivity("Quote Accepted - Quote ID: " . $id);
            $quote_data = (int) $validQuote;
...
if (isset($_SESSION["adminid"])) {
    ...
    $result = select_query("tblquotes", "", ["id" => $id, "userid" => $_SESSION["uid"], "stage" => ["sqltype" => "NEQ", "value" => "Draft"]]);
}
...
$quoteitems = [];
$result = select_query("tblquoteitems", "quantity,description,unitprice,discount,taxable", ["quoteid" => $id], "id", "ASC");

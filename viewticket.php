...
if ($ticket) {
    ...
    if ($whmcs->get_req_var("feedback") && $tickets->getDepartmentFeedbackNotifications()) {
        ...
        $showclosebutton = !in_array($ticket->status, $closedTicketStatuses);
        ...
        $replies = $ascreplies = [];
        $ascreplies[] = ["id" => "", "userid" => $ticket->userid, "contactid" => $ticket->contactid, "name" => $ticket->getRequestorName(), "email" => $ticket->getRequestorEmail(), "ipaddress" => $ticket->ipaddress, "requestor" => ["name" => $ticket->getRequestorName(), "email" => $ticket->getRequestorEmail(), "type" => $ticket->getRequestorType(), "type_normalised" => WHMCS\Utility\Status::normalise($ticket->getRequestorType())], "admin" => $ticket->postedByAnAdmin(), "date" => fromMySQLDate($ticket->date, 1, 1), "message" => $message, "attachments" => $ticket->getAttachmentsForDisplay(), "attachments_removed" => $ticket->attachmentsRemoved, "rating" => $ticket->rating];
